...
- **Снижение фоновой нагрузки на роутер из LuCI**:
  - В `runtimeUiState.service.ts` интервал ожидания (idle poll) установлен на 2000 мс (вместо 1000 мс), что сокращает число форков системных утилит на роутере вдвое при сохранении быстрого 500 мс отклика во время активных операций.
