...
- **In-process исполнение без форков**: команды выполняются напрямую в текущем процессе ucode через `include(module)` без вызова внешнего шелла (`/bin/sh`) и повторного запуска интерпретатора, что снижает нагрузку на CPU роутера и обеспечивает мгновенный отклик.
