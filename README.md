...
- 🎛️ **Выбор сборки sing-box**: `tiny` (минимальный размер flash), `stable` (стандартная) и `extended` (поддержка xHTTP).
...
- **Наследование реальной задержки**: карточка группы URLTest на Дашборде отображает актуальную задержку текущего активного узла, даже если история проверок самой группы пуста.
- **Отказоустойчивый fallback при фильтрации**: если пользовательские фильтры серверов (по странам, протоколам или регулярным выражениям) отсеяли все доступные узлы подписки, генератор sing-box автоматически откатывается ко всем доступным кандидатам, предотвращая падение службы и потерю доступа к сети.
