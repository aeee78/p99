...
Build P99 IPK and APK packages. The version must use x.y.z format.
EOF
}
...
SDK_CACHE_DIR="${SDK_CACHE_DIR:-$SDK_CACHE_BASE/p99/openwrt-sdk}"
...
BACKEND_DESCRIPTION="Rule-based P99 backend with hybrid sing-box + zapret orchestration"
APP_DESCRIPTION="Rule-based P99 LuCI app with hybrid sing-box + zapret orchestration"
I18N_DESCRIPTION="Translation for luci-app-p99 - Русский (Russian)"
MAINTAINER="aeee78 <235684779+aeee78@users.noreply.github.com>"
PROJECT_URL="https://github.com/aeee78/p99"
...
APP_DEPENDS_IPK="libc, luci-base, p99"
APP_DEPENDS_APK="libc luci-base p99"
...
extract_sdk() {
  ...
  local marker_file="$destination/.p99-sdk-url"
  local temp_dir
...
build_backend_root() {
  ...
  make_dir "$output_root/usr/lib/p99"
  make_dir "$output_root/usr/share/p99/defaults"
  ...
  sed -i -e "s/__COMPILED_VERSION_VARIABLE__/${RELEASE_VERSION}/g" \
    "$output_root/usr/lib/p99/core/constants.uc"
...
build_app_root() {
  ...
  cp -a "$ROOT_DIR/luci-app-p99/htdocs/." "$output_root/www/"
  cp -a "$ROOT_DIR/luci-app-p99/root/." "$output_root/"
  sed -i -e "s/__COMPILED_VERSION_VARIABLE__/${RELEASE_VERSION}/g" \
    "$output_root/www/luci-static/resources/view/p99/main.js"
...
build_i18n_root() {
  local output_root="$1"
  local po2lmo_bin="$2"
  local lmo_path="$output_root/usr/lib/lua/luci/i18n/p99.ru.lmo"

  rm -rf "$output_root"
  make_dir "$output_root/etc/uci-defaults"
  make_dir "$(dirname "$lmo_path")"

  cat > "$output_root/etc/uci-defaults/luci-i18n-p99-ru" <<'EOF'
uci set luci.languages.ru='Русский (Russian)'; uci commit luci
EOF

  "$po2lmo_bin" "$ROOT_DIR/luci-app-p99/po/ru/p99.po" "$lmo_path"
...
write_i18n_ipk_control() {
  ...
  cat > "$control_dir/control" <<EOF
Package: luci-i18n-p99-ru
Version: ${RELEASE_VERSION}
Depends: libc, luci-app-p99
License: GPL-2.0-or-later
Section: luci
URL: ${PROJECT_URL}
Maintainer: ${MAINTAINER}
...
#!/usr/bin/ucode
if (getenv("IPKG_INSTROOT") == null || getenv("IPKG_INSTROOT") == "")
    exit(system("P99_LIB=/usr/lib/p99 ucode -L /usr/lib/p99 /usr/lib/p99/config/migration.uc migrate && /usr/bin/p99 package_postinst"));
exit(0);
EOF
...
if (getenv("IPKG_INSTROOT") == null || getenv("IPKG_INSTROOT") == "")
	system("/usr/bin/p99 package_prerm remove >/dev/null 2>&1");
...
. ${IPKG_INSTROOT}/lib/functions.sh
export root="${IPKG_INSTROOT}"
export pkgname="luci-i18n-p99-ru"
add_group_and_user
default_postinst
EOF
...
main() {
  ...
  if ! flock -n 9; then
    echo "Another P99 package build is already running" >&2
    exit 1
  fi
  output_dir="$OUTPUT_DIR"
  mkdir -p "$output_dir"
  rm -f "$output_dir"/p99_* "$output_dir"/luci-app-p99_* "$output_dir"/luci-i18n-p99-ru_*
  ...
  build_ipk_package \
    ...
    "$backend_root" \
    "$backend_control" \
    "$output_dir/p99_${RELEASE_VERSION}.ipk"
  ...
  build_ipk_package \
    ...
    "$app_root" \
    "$app_control" \
    "$output_dir/luci-app-p99_${RELEASE_VERSION}.ipk"
  ...
  build_ipk_package \
    ...
    "$i18n_root" \
    "$i18n_control" \
    "$output_dir/luci-i18n-p99-ru_${RELEASE_VERSION}.ipk"

  generate_apk_metadata_files "p99" "$backend_root" "/etc/config/p99"
  generate_apk_metadata_files "luci-app-p99" "$app_root"
  generate_apk_metadata_files "luci-i18n-p99-ru" "$i18n_root"
  ...
  build_apk_package \
    ...
    "$apk_scripts" \
    "backend" \
    "$output_dir/p99_${RELEASE_VERSION}.apk" \
    "$MAINTAINER"
  ...
  build_apk_package \
    ...
    "$apk_scripts" \
    "app" \
    "$output_dir/luci-app-p99_${RELEASE_VERSION}.apk" \
    "$MAINTAINER"
  ...
  build_apk_package \
    ...
    "$apk_scripts" \
    "i18n" \
    "$output_dir/luci-i18n-p99-ru_${RELEASE_VERSION}.apk" \
    "$MAINTAINER"

  verify_ipk_metadata "$output_dir/p99_${RELEASE_VERSION}.ipk" "p99" "$RELEASE_VERSION"
  verify_ipk_metadata "$output_dir/luci-app-p99_${RELEASE_VERSION}.ipk" "luci-app-p99" "$RELEASE_VERSION"
  verify_ipk_metadata "$output_dir/luci-i18n-p99-ru_${RELEASE_VERSION}.ipk" "luci-i18n-p99-ru" "$RELEASE_VERSION"
  verify_apk_metadata "$apk_bin" "$output_dir/p99_${RELEASE_VERSION}.apk" "p99" "$APK_INTERNAL_VERSION"
  verify_apk_metadata "$apk_bin" "$output_dir/luci-app-p99_${RELEASE_VERSION}.apk" "luci-app-p99" "$APK_INTERNAL_VERSION"
  verify_apk_metadata "$apk_bin" "$output_dir/luci-i18n-p99-ru_${RELEASE_VERSION}.apk" "luci-i18n-p99-ru" "$APK_INTERNAL_VERSION"
