...
import { P99ShellMethods } from '../shell';

interface IGetDashboardSectionsResponse {
  success: boolean;
  data: P99.OutboundGroup[];
}
...
function getDisplayName(section: P99.ConfigSection) {
  return section.label || section['.name'];
}

function getSettingsSection(configSections: P99.ConfigSection[]) {
  return configSections.find((section) => section['.type'] === 'settings');
}

function getClashApiSecret(configSections: P99.ConfigSection[]) {
...
): Promise<P99.MethodResponse<ClashAPI.Proxies>> {
...
function ownedChildSections(
  parent: P99.ConfigSection,
  children: P99.ConfigSection[],
) {
  return children.filter(
    (section): section is P99.ConfigSection =>
...
function childSectionsByOwner(
  children: P99.ConfigSection[],
  ownerKey: keyof P99.ConfigSection,
  ownerValue: string,
) {
...
function getManualProxyLinks(section: P99.ConfigSection) {
  return getListValues(section.selector_proxy_links);
}

function getConnectionInterfaces(section: P99.ConfigSection) {
  const values = getListValues(section.interfaces);
  return values.length ? values : getListValues(section.interface);
}

function getJsonOutbounds(section: P99.ConfigSection) {
...
function hasSubscriptionSources(section: P99.ConfigSection) {
...
function hasConfiguredUrlTestList(section: P99.ConfigSection) {
  return getListValues(section.urltests).length > 0;
}

function hasConfiguredPriorityList(section: P99.ConfigSection) {
  return getListValues(section.priority_groups).length > 0;
}

function getUrlTestIds(section: P99.ConfigSection) {
...
function isUrlTestEnabled(section: P99.ConfigSection) {
  return getUrlTestIds(section).length > 0;
}

function shouldUseProxyGroup(section: P99.ConfigSection) {
  return (
...
function getSectionProxyConfigType(section: P99.ConfigSection) {
...
function getJsonOutboundDisplayName(section: P99.ConfigSection) {
  try {
...
function buildManualLinkByCode(section: P99.ConfigSection) {
...
function isSelectorOutbound(outbound: P99.Outbound) {
...
function sortUrlTestMembers(outbounds: P99.UrlTestMember[]) {
  return outbounds
    .map((outbound, index) => ({ outbound, index }))
    .sort((left, right) => {
...
function getUrlTestConfigs(section: P99.ConfigSection): UrlTestConfig[] {
...
function getPriorityGroupIds(section: P99.ConfigSection) {
  return getListValues(section.priority_groups);
}

function getPriorityConfigs(section: P99.ConfigSection): PriorityConfig[] {
...
function metadataMatchesCurrentSource(
  sectionName: string,
  sourceCount: number,
  metadata: P99.SubscriptionMetadata,
) {
  const legacyMetadata = metadata as P99.SubscriptionMetadata & {
    source_index?: number;
    ...
    configSections
      ...
      .map(async (section) => {
        ...
        if (isConnectionAction(sectionAction) && shouldUseProxyGroup(section)) {
          ...
          const latencyTestTimeoutStr =
            getSettingsSection(configSections)?.latency_test_timeout || '2000';
          const latencyTestTimeoutNum = Number(latencyTestTimeoutStr) || 2000;
