...
const RUNTIME_UI_STATE_IDLE_POLL_INTERVAL_MS = 5000;
const RUNTIME_UI_STATE_ACTIVE_POLL_INTERVAL_MS = 500;
type RuntimeUiStateListener = (uiState: P99.UiState) => void;

let runtimeUiStateRefreshPromise: Promise<P99.UiState | undefined> | null =
  null;
let lastRuntimeUiStateRefreshAt = 0;
let lastRuntimeUiState: P99.UiState | undefined;
...
function hasRunningAction(uiState: P99.UiState) {
...
function notifyRuntimeUiStateListeners(uiState: P99.UiState) {
...
export async function refreshRuntimeUiState({
  force = false,
}: { force?: boolean } = {}): Promise<P99.UiState | undefined> {
  ...
  const promise = P99ShellMethods.getUiState()
    .then((response) => {
      if (!response.success) {
        return undefined;
      }
