...
describe('LogNotificationDeduper', () => {
  it('accepts error, fatal, and component update log lines', () => {
    expect(isErrorLogLine('p99: [info] ok')).toBe(false);
    expect(isErrorLogLine('p99: [error] failed')).toBe(true);
    expect(isErrorLogLine('p99: [fatal] failed')).toBe(true);
    expect(
      isErrorLogLine(
    ...
    expect(getP99LogNotification('p99: [info] ok')).toBeNull();
  });
  ...
  it('keeps the full log line as the replay key', () => {
    expect(getLogNotificationKey('  Jun 06 p99: [error] failed  ')).toBe(
      'Jun 06 p99: [error] failed',
    );
  });
});
