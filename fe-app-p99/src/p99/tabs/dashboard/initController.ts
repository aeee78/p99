...
import { CustomP99Methods, P99ShellMethods } from '../../methods';
import {
  logger,
  markUiActionOwned,
...
function setLatencyFetching(
  ...
  fetching: boolean,
  local = false,
  progress?: P99.LatencyActionProgress,
) {
  if (local || !fetching) {
...
async function completeSubscriptionUpdateJob(
  jobId: string,
  sectionName: string,
  response: P99.MethodResponse<P99.SubscriptionUpdateJobState>,
) {
  if (pageUnloading) {
...
async function followSubscriptionUpdateState(
  state: P99.SubscriptionUpdateJobState,
) {
  ...
  try {
    const response = state.running
      ? await P99ShellMethods.waitSubscriptionUpdateJob(jobId)
      : ({
          success: true,
          data: state,
...
async function followLatencyTestState(state: P99.LatencyActionState) {
  const jobId = state.job_id;
  const sectionName = state.section || '';
...
function followDashboardActionsFromUiState(uiState: P99.UiState) {
...
function getInitialLatencyProgress(
  ...
  tag: string,
): P99.LatencyActionProgress | undefined {
...
function renderDetailsMemberName(member: P99.UrlTestMember) {
...
function renderUrlTestSelectedValue(info: P99.UrlTestInfo) {
...
function renderUrlTestInfoModal(outbound: P99.Outbound) {
...
function renderUrlTestEditorModal(outbound: P99.Outbound) {
  const info = outbound.urlTestInfo;
  if (!info) return;
...
function handleShowUrlTestInfo(outbound: P99.Outbound) {
...
function renderPrioritySelectedValue(info: P99.PriorityInfo) {
  const selectedMember = info.outbounds.find((member) => member.selected);
  const selectedName =
    selectedMember?.displayName || info.selectedName || info.selectedCode || '';
  const name = formatUrlTestModalValue(selectedName);
...
function renderPriorityMemberName(member: P99.PriorityMember) {
...
function renderPriorityInfoModal(outbound: P99.Outbound) {
...
function handleShowPriorityInfo(outbound: P99.Outbound) {
...
async function handleUpdateSubscription(section: P99.OutboundGroup) {
  if (
...
async function renderServicesInfoWidget() {
  ...
  const renderedWidget = renderWidget({
    ...
    items: [
      {
        key: 'P99',
        value: servicesInfoWidget.data.p99Running
          ? _('✔ Running')
          : _('✘ Stopped'),
        attributes: {
          class: servicesInfoWidget.data.p99Running
