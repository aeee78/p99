...
export async function runByedpiCheck() {
  ...
  const items: Array<IDiagnosticsChecksItem> = [
    ...
    {
      state:
        unexpectedRuntime || !p99RuntimeReady
          ? 'error'
          : runtimeUnstable
            ? 'warning'
            : 'success',
      key: hasByedpiRules
        ? runtimeUnstable
          ? _('P99-managed ciadpi runtime has restarted')
          : p99RuntimeReady
            ? _('P99-managed ciadpi runtime is ready')
            : _('P99-managed ciadpi runtime is not ready')
        : unexpectedRuntime
          ? _('Unexpected P99-managed ciadpi runtime is running')
          : _('P99-managed ciadpi runtime is not running'),
    ...
    },
    {
      ...
      key: standaloneServiceRunning
        ? hasByedpiRules
          ? _('Standalone ByeDPI is active together with P99 ByeDPI rules')
