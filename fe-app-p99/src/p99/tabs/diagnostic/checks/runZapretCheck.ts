...
export async function runZapretCheck() {
  ...
  const items: Array<IDiagnosticsChecksItem> = [
    ...
    },
    {
      ...
      key: hasZapretRules
        ? p99RuntimeReady
          ? _('P99-managed nfqws runtime is ready')
          : _('P99-managed nfqws runtime is not ready')
        : unexpectedRuntime
          ? _('Unexpected P99-managed nfqws runtime is running')
          : _('P99-managed nfqws runtime is not running'),
    ...
    },
    {
      ...
      key: standaloneServiceRunning
        ? hasZapretRules
          ? _('Standalone Zapret is active together with P99 Zapret rules')
