...
import { P99_LUCI_APP_VERSION } from '../../../constants';
...
function isMutatingServiceActionLoading() {
  return (
    ...
    isServiceTransitionStatus(store.get().servicesInfoWidget.data.p99Status)
  );
}
...
function getP99StatusText(running: boolean, enabled: boolean) {
  if (running) {
...
function setDisplayedP99Running(running: boolean) {
...
async function waitForP99RunningState(expectedRunning: boolean) {
...
async function followServiceActionState(state: P99.ServiceActionState) {
...
function followServiceActionsFromUiState(uiState?: P99.UiState) {
  if (!uiState) {
    return;
  }
