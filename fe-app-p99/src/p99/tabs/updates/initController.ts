...
let componentUpdateCheckCacheSnapshot: P99.ComponentUpdateCheckCache | null =
  null;
let componentUpdateCheckCachePromise: Promise<P99.ComponentUpdateCheckCache> | null =
  null;
...
function shouldShowInstallAfterCheck(component: P99.ComponentName) {
  const status = getVisibleCheckResult(component)?.status;

  return status === 'outdated' || status === 'dev';
}

function getVisibleCheckResult(component: P99.ComponentName) {
  if (
    !shouldExposeCheckResults({
...
function getLatestVersion(component: P99.ComponentName) {
...
function getGitHubReleaseUrl(component: P99.ComponentName) {
  const checkResult = getVisibleCheckResult(component);
...
function isServiceRuntimeActionLoading() {
  ...
  return (
    ...
    isServiceTransitionStatus(state.servicesInfoWidget.data.p99Status)
  );
}
...
function resetCheckResult(component: P99.ComponentName) {
  setCheckResult(component, null, '');
}

function applyCachedCheckResults(results: P99.ComponentActionResult[]) {
...
function loadComponentUpdateCheckCache({ force = false } = {}) {
  ...
  const promise = P99ShellMethods.componentUpdateCheckCache()
    .then((response) =>
      response.success
        ? response.data
        : ({
            enabled: false,
            results: [],
          } satisfies P99.ComponentUpdateCheckCache),
    )
    .then((cache) => {
...
function patchSystemInfoAfterMutation(result: P99.ComponentActionResult) {
  const systemInfo = store.get().diagnosticsSystemInfo;
...
async function applyCompletedComponentAction({
  ...
  key: UpdatesActionKey;
  result: P99.ComponentActionResult;
  notify: boolean;
}) {
...
async function completeComponentActionJob(
  key: UpdatesActionKey,
  jobId: string,
  response: P99.MethodResponse<P99.ComponentActionResult>,
) {
  if (pageUnloading) {
    setActionLoading(key, false);
    return;
...
async function followComponentActionState(state: P99.ComponentActionResult) {
  ...
  try {
    const response = state.running
      ? await P99ShellMethods.waitComponentActionJob(
          jobId,
          state.component,
          state.action,
...
function handleComponentUiState(uiState: P99.UiState) {
...
function getComponentCards(): ComponentCard[] {
  ...
  // P99 X exposes only the two mirror-backed sing-box variants.
  if (!singBoxTiny) {
    singBoxActions.push({
  ...
  return [
    {
      ...
      version: systemInfoLoading
        ? _('Loading...')
        : normalizeCompiledVersion(systemInfo.p99_version),
...
function applyComponentUpdateCheckCache(
  componentUpdateCheckCache: P99.ComponentUpdateCheckCache,
) {
