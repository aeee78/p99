...
const syncDirs = [
  {
    local: path.resolve(
      process.env.LOCAL_DIR_FE ??
        '../luci-app-p99/htdocs/luci-static/resources/view/p99',
    ),
    remote:
      process.env.REMOTE_DIR_FE ?? '/www/luci-static/resources/view/p99',
  },
  {
    local: path.resolve(
      process.env.LOCAL_DIR_BIN ?? '../p99/files/usr/bin/',
    ),
    remote: process.env.REMOTE_DIR_BIN ?? '/usr/bin/',
  },
  {
    local: path.resolve(
      process.env.LOCAL_DIR_LIB ?? '../p99/files/usr/lib/',
    ),
    remote: process.env.REMOTE_DIR_LIB ?? '/usr/lib/p99/',
  },
  {
    local: path.resolve(
      process.env.LOCAL_DIR_INIT ?? '../p99/files/etc/init.d/',
    ),
