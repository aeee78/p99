...
"require ui";
"require uci";
"require view.p99.local_devices as localDevices";
"require view.p99.main as main";

const UCI_PACKAGE = main.P99_UCI_PACKAGE;
const ACTION_PROVIDERS_AVAILABILITY_EVENT =
  main.P99_ACTION_PROVIDERS_AVAILABILITY_EVENT ||
  "p99:action-providers-availability";
...
const NFQWS_VALIDATION_COMMAND = "/usr/bin/p99";
...
function validateSubscriptionUrlEntry(_section_id, value) {
  ...
  try {
    ...
    if (url.protocol !== "https:" && url.protocol !== "http:") {
      return _("Subscription URL must use HTTP or HTTPS");
    }
  } catch (_error) {
...
function getNfqws2ForbiddenTokenInfo(token, index, nextToken) {
  const configFileMessage = _(
    "External nfqws2 config files bypass P99 queue management and explicit validation.",
  );
  const hostSelectionMessage = _(
  ...
  const placeholderMessage = _(
    "Zapret2 hostlist templates are not supported here because P99 does not expand them for per-rule NFQWS2 strategies.",
  );
  const queueMessage = _(
    "The NFQUEUE number is assigned by P99 for each rule and must not be overridden here.",
  );
  ...
  const fuzzMessage = _(
    "Fuzzing is not supported here because P99 needs deterministic runtime validation.",
  );
  const interceptMessage = _(
    "Disabling interception is incompatible with action=zapret2 because P99 sends matched traffic through NFQUEUE.",
  );
  const daemonMessage = _(
    "P99 manages the nfqws2 process lifecycle itself, so daemon mode is not allowed here.",
  );
  const dryRunMessage = _(
...
function getByedpiControlledTokenInfo(token) {
  const listenMessage = _(
    "ByeDPI listen address and port are assigned by P99 and must not be set in the strategy.",
  );
  const transparentMessage = _(
    "Transparent proxy mode is incompatible with action=byedpi because P99 connects to ciadpi through SOCKS.",
  );
  const daemonMessage = _(
    "P99 manages the ciadpi process lifecycle itself, so daemon mode is not allowed here.",
  );
  const pidfileMessage = _(
    "P99 manages ciadpi pid files itself, so pidfile options are not allowed here.",
  );
  const exitMessage = _(
