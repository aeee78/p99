...
const CONFIG_NAME = getenv("P99_CONFIG_NAME") || constants.P99_CONFIG_NAME || "p99";
const BIN_PATH = getenv("P99_BIN") || constants.P99_BIN || "/usr/bin/p99";
const SERVICE_INIT = getenv("P99_SERVICE_INIT") || constants.P99_SERVICE_INIT || "/etc/init.d/p99";
const P99_VERSION = getenv("P99_VERSION") || constants.P99_VERSION || "";
const P99_RELEASE_REPO = getenv("P99_RELEASE_REPO") || constants.P99_RELEASE_REPO || "aeee78/p99";
const P99_MIRROR_BASE_URL = getenv("P99_MIRROR_BASE_URL") || constants.P99_MIRROR_BASE_URL || "";
const RUNTIME_STATE_DIR = getenv("P99_RUNTIME_STATE_DIR") || "/var/run/p99";
const SYSTEM_INFO_CACHE_FILE = getenv("P99_SYSTEM_INFO_CACHE_FILE") || RUNTIME_STATE_DIR + "/system-info.json";
...
const SB_MANAGED_SERVICE_MARKER = getenv("SB_MANAGED_SERVICE_MARKER") || constants.SB_MANAGED_SERVICE_MARKER || "P99 managed sing-box service for binary variants";
const TORRSERVER_DIRECT_INIT = getenv("P99_TORRSERVER_DIRECT_INIT") || "/etc/init.d/p99-torrserver-direct";
const TORRSERVER_DIRECT_UC = LIB_DIR + "/torrserver/direct.uc";

let tmp_dir = "";
let lock_held = false;
let p99_was_running = false;
let p99_stopped_for_sing_box_change = false;
...
function log_message(message, level) {
    ...
    command_success_from_args([ "logger", "-t", "p99", "[" + level + "] " + as_string(message) ]);
}
...
function cleanup_stale_tmp_files() {
    command_success_from_args([ "find", "/tmp", "-maxdepth", "1", "-type", "d", "-name", "p99-updates.*", "-mmin", "+" + as_string(TMP_STALE_TTL_MINUTES), "-exec", "rm", "-rf", "{}", "+" ]);
    command_success_from_args([ "find", "/tmp", "-maxdepth", "1", "-type", "f", "(", "-name", "p99-updates-command.*", "-o", "-name", "p99-updates-http.*", ")", "-mmin", "+" + as_string(TMP_FILE_STALE_TTL_MINUTES), "-delete" ]);
}
...
function init_tmp_dir() {
    ...
    tmp_dir = trim(command_output_from_args([ "mktemp", "-d", "/tmp/p99-updates.XXXXXX" ]));
    if (tmp_dir == "") {
        tmp_dir = "/tmp/p99-updates." + owner_pid();
...
function make_tmp_file(prefix) {
    init_tmp_dir();
    let base = tmp_dir != "" ? tmp_dir + "/" + as_string(prefix) + ".XXXXXX" : "/tmp/p99-updates-" + as_string(prefix) + ".XXXXXX";
...
function restart_p99_after_failed_sing_box_change() {
    if (!p99_stopped_for_sing_box_change || !p99_was_running || !file_exists(SERVICE_INIT))
        return;
    updates_log("Restarting P99 after failed sing-box component change");
...
function action_fail(component, action, message, current_version, latest_version, status, release_url) {
    ...
    restart_p99_after_failed_sing_box_change();
...
function run_logged(description, command) {
    ...
    if (output_file == "")
        output_file = "/tmp/p99-updates-command." + owner_pid();
...
function latest_p99_release_json() {
    if (P99_MIRROR_BASE_URL != "") {
        let response = http_get(P99_MIRROR_BASE_URL + "/p99/updates/latest.json");
        if (response == "")
            return "";
        return response;
    }

    let parts = split(P99_RELEASE_REPO, "/");
    if (length(parts) != 2 || as_string(parts[0]) == "" || as_string(parts[1]) == "")
        return "";
    return fetch_github_release_json(parts[0], parts[1]);
}

function p99_release_url(value) {
    value = as_string(value);
    if (P99_MIRROR_BASE_URL != "" && substr(value, 0, 1) == "/")
        return P99_MIRROR_BASE_URL + value;
    return value;
}

function latest_p99_version() {
    let response = latest_p99_release_json();
    if (response == "")
        return "";
    return trim(helper_output_input(response, "object-get-default", [ "tag_name", "" ]));
}

function fetch_p99_latest_release_metadata() {
    let response = latest_p99_release_json();
    if (response == "")
        return "";
...
function write_p99_latest_version_cache(value, timestamp) {
    if (as_string(value) == "")
        return;
    write_file("/tmp/p99.latest-version.cache", as_string(value) + "\n" + as_string(timestamp) + "\n");
}
...
function clear_version_caches() {
    remove_file("/tmp/p99.latest-version.cache");
    remove_file(SYSTEM_INFO_CACHE_FILE);
    remove_file("/tmp/p99/system-info.json");
}
...
function install_managed_sing_box_service_script() {
    let tmp = "/etc/init.d/sing-box.p99." + owner_pid();
...
function p99_status_running_with_timeout() {
    init_tmp_dir();
    let output_file = make_tmp_file("p99-status");
    if (output_file == "")
        return false;
...
function capture_p99_running_state() {
    p99_was_running = file_exists(BIN_PATH) && p99_status_running_with_timeout();
}

function restart_p99_after_successful_change() {
    if (!file_exists(SERVICE_INIT))
        return;
    if (!p99_was_running) {
        updates_log("P99 was not running before component change; restart skipped");
        prepare_sing_box_service_disabled();
        return;
    }
    run_logged("Restarting P99 after successful component change", command_from_args([ SERVICE_INIT, "restart" ]));
}

function stop_p99_before_sing_box_change() {
    if (p99_stopped_for_sing_box_change)
        return;
    p99_stopped_for_sing_box_change = true;

    if (p99_was_running && file_exists(SERVICE_INIT))
        run_logged("Stopping P99 before sing-box package change", command_from_args([ SERVICE_INIT, "stop" ]));

    if (p99_was_running && file_exists(BIN_PATH))
        command_success_from_args([ BIN_PATH, "restore_dnsmasq" ]);

    prepare_sing_box_service_disabled();
}

function wait_p99_running_after_sing_box_change() {
    if (!p99_was_running)
        return true;
    ...
    let waited = 0;
    while (waited < 60) {
        if (p99_status_running_with_timeout()) {
...
function install_zapret_like(component, action, runtime_module, resolve_fn, label) {
    ...
    restart_p99_after_successful_change();
...
function install_zapret_manager(action) {
    ...
    let default_url = "https://raw.githubusercontent.com/stressozz/Zapret-Manager/main/Zapret-Manager.sh";
    let fallback_url = "https://cdn.jsdelivr.net/gh/stressozz/Zapret-Manager@main/Zapret-Manager.sh";
    let manager_url = P99_MIRROR_BASE_URL != "" ? P99_MIRROR_BASE_URL + "/zapret-manager/proxy/raw.githubusercontent.com/stressozz/Zapret-Manager/main/Zapret-Manager.sh" : default_url;
    let manager_file = tmp_dir + "/Zapret-Manager.sh";
    let current_version = file_exists("/usr/bin/zms") ? "installed" : "not installed";

    let downloaded = download_with_retry(manager_url, manager_file, "Zapret-Manager");
    if (!downloaded && manager_url != fallback_url) {
        manager_url = fallback_url;
        downloaded = download_with_retry(manager_url, manager_file, "Zapret-Manager fallback");
    }
    if (!downloaded)
        action_fail(component, action, "Failed to download Zapret-Manager", current_version);

    let source = read_file(manager_file);
    let matched = match(source, /ZAPRET_MANAGER_VERSION="([^"]+)"/);
    let latest_version = matched != null ? as_string(matched[1]) : "latest";
    let wrapper = "#!/bin/sh\nexec sh <(wget -q -O - " + shell_quote(default_url) + ") \"$@\"\n";
    ...
    clear_version_caches();
    action_success(component, action, "Zapret-Manager has been installed", current_version,
        latest_version, 1, "latest", default_url);
}
...
function move_file_portable(source_path, target_path) {
    ...
    let staged_path = as_string(target_path) + ".p99-move." + owner_pid();
...
function install_staged_file(source_path, target_path, mode) {
    let staged_path = as_string(target_path) + ".p99-new." + owner_pid();
    remove_file(staged_path);
...
function install_sing_box_extended_package(action) {
    ...
    stop_p99_before_sing_box_change();
    prepare_sing_box_package_service_install();
    ...
    if (current_variant == "extended" || current_variant == "extended-compressed") {
        if (file_exists("/usr/bin/sing-box")) {
            backup_binary = "/usr/bin/sing-box.p99-backup." + owner_pid();
        ...
        if (file_exists("/usr/lib/libcronet.so")) {
            cronet_touched = true;
            backup_cronet = "/usr/lib/libcronet.so.p99-backup." + owner_pid();
    ...
    write_sing_box_variant_state("extended", new_version);
    restart_p99_after_successful_change();
    if (!wait_p99_running_after_sing_box_change()) {
        ...
        if (restore_sing_box_after_failed_extended_package_install(current_variant, backup_binary, backup_cronet, previous_marker, previous_version_state, package_file, cronet_touched)) {
            remove_file(backup_binary);
            remove_file(backup_cronet);
            action_fail("sing_box", action, "sing-box-extended package was installed but P99 did not start cleanly; previous sing-box variant was restored", current_version, latest_version);
        }
        action_fail("sing_box", action, "sing-box-extended package was installed but P99 did not start cleanly and previous sing-box variant could not be restored", current_version, latest_version);
    }
...
function install_sing_box_extended(action, compressed) {
    ...
    if (!wait_p99_running_after_sing_box_change()) {
        ...
        if (restore_sing_box_after_failed_extended_install(current_variant, backup_binary, backup_cronet, previous_marker, previous_version_state, archive_file, cronet_touched)) {
            remove_file(backup_binary);
            remove_file(backup_cronet);
            action_fail("sing_box", action, label + " was installed but P99 did not start cleanly; previous sing-box variant was restored", current_version, latest_version);
        }
        action_fail("sing_box", action, label + " was installed but P99 did not start cleanly and previous sing-box variant could not be restored", current_version, latest_version);
    }
...
function install_package_sing_box(action, tiny) {
    ...
    if (file_exists("/usr/bin/sing-box")) {
        backup_binary = backup_on_tmpfs ? tmp_dir + "/sing-box.p99-backup" :
            "/usr/bin/sing-box.p99-backup." + owner_pid();
        if (!move_file_to_backup("/usr/bin/sing-box", backup_binary))
            action_fail("sing_box", action, "Failed to backup current sing-box binary", current_version, latest_version);
    }
    if (file_exists("/usr/lib/libcronet.so")) {
        cronet_touched = true;
        backup_cronet = backup_on_tmpfs ? tmp_dir + "/libcronet.so.p99-backup" :
            "/usr/lib/libcronet.so.p99-backup." + owner_pid();
        if (!move_file_to_backup("/usr/lib/libcronet.so", backup_cronet)) {
    ...
    restart_p99_after_successful_change();
    if (!wait_p99_running_after_sing_box_change())
        fail_package_sing_box_install(action, tiny, "was installed, but P99 did not start cleanly", new_version, latest_version,
            package_name, previous_variant, backup_binary, backup_cronet, previous_marker, previous_version_state, cronet_touched);
    remove_file(backup_binary);
    remove_file(backup_cronet);
    clear_version_caches();
...
function check_p99() {
    let metadata = fetch_p99_latest_release_metadata();
    ...
    if (latest_version == "unknown")
        action_fail("p99", "check_update", "Failed to check P99 updates", P99_VERSION, latest_version);

    write_p99_latest_version_cache(latest_version, now_seconds());
    if (!helper_success("p99-release-version-valid", [ P99_VERSION ])) {
        updates_log("P99 current version is not a release version (" + P99_VERSION + ")");
        action_success("p99", "check_update", "Installed version is newer than release", P99_VERSION, latest_version, 0, "dev", release_url);
    }

    let compare = trim(helper_output("p99-release-version-compare", [ P99_VERSION, latest_version ]));
    if (compare == "")
        action_fail("p99", "check_update", "Failed to compare P99 versions", P99_VERSION, latest_version);
    let status = status_from_compare(int(compare));
    if (status == "")
        action_fail("p99", "check_update", "Failed to compare P99 versions", P99_VERSION, latest_version);
    if (status == "latest") {
        updates_log("P99 is already up to date (" + P99_VERSION + ")");
        action_success("p99", "check_update", "Latest version is installed", P99_VERSION, latest_version, 0, status, release_url);
    }
    if (status == "outdated") {
        updates_log("P99 update found: " + P99_VERSION + " -> " + latest_version);
        action_success("p99", "check_update", "Update is available", P99_VERSION, latest_version, 0, status, release_url);
    }
    updates_log("P99 installed version is newer than upstream release: " + P99_VERSION + " -> " + latest_version);
    action_success("p99", "check_update", "Installed version is newer than release", P99_VERSION, latest_version, 0, status, release_url);
}

function resolve_p99_release(latest_version) {
    let release_json = latest_p99_release_json();
    if (release_json == "")
        return null;
    let asset_ext = is_apk() ? "apk" : "ipk";
    let i18n_required = pkg_is_installed("luci-i18n-p99-ru") ? "1" : "0";
    let plan = trim(helper_output_input(release_json, "p99-release-plan", [ latest_version, asset_ext, i18n_required ]));
    let fields = split(plan, "\t");
    if (length(fields) < 7 || as_string(fields[1]) == "" || as_string(fields[2]) == "" || as_string(fields[3]) == "" || as_string(fields[4]) == "")
        return null;
    return {
        release_url: p99_release_url(fields[0]),
        backend_name: fields[1],
        backend_url: p99_release_url(fields[2]),
        app_name: fields[3],
        app_url: p99_release_url(fields[4]),
        i18n_name: fields[5],
        i18n_url: p99_release_url(fields[6])
    };
}

function install_p99() {
    let latest_version = latest_p99_version();
    if (latest_version == "")
        latest_version = "unknown";
    if (latest_version == "unknown")
        action_fail("p99", "install", "Failed to resolve P99 release", P99_VERSION, latest_version);

    write_p99_latest_version_cache(latest_version, now_seconds());
    init_tmp_dir() || action_fail("p99", "install", "Failed to create temporary directory", P99_VERSION, latest_version);
    updates_log("Resolving P99 release " + latest_version + " packages");
    let release = resolve_p99_release(latest_version);
    if (release == null)
        action_fail("p99", "install", "Failed to resolve P99 release packages", P99_VERSION, latest_version);
    ...
    if (!download_with_retry(release.backend_url, backend_file, release.backend_name) ||
        ...
        action_fail("p99", "install", "Failed to download P99 release packages", P99_VERSION, latest_version);

    if (!run_logged("Installing LuCI app package " + release.app_name, pkg_install_files_command([ app_file ])))
        action_fail("p99", "install", "Failed to install LuCI app package", P99_VERSION, latest_version);
    if (i18n_file != "" && !run_logged("Installing LuCI Russian i18n package " + release.i18n_name, pkg_install_files_command([ i18n_file ])))
        action_fail("p99", "install", "Failed to install LuCI Russian i18n package", P99_VERSION, latest_version);
    if (!run_logged("Installing P99 package " + release.backend_name, pkg_install_files_command([ backend_file ])))
        action_fail("p99", "install", "Failed to install P99 package", P99_VERSION, latest_version);
    ...
    restart_p99_after_successful_change();
    clear_version_caches();
    let new_version = installed_package_version("p99");
    if (new_version == "")
        new_version = latest_version;
    updates_log("P99 updated to " + new_version);
    action_success("p99", "install", "P99 has been installed", new_version, latest_version, 1, "latest", release.release_url);
}
...
function set_packet_steering(action) {
    let target_mode = action == "restore" ? "0" : "1";
    let current_mode = trim(uci_core.get("network.@globals[0].packet_steering")) == "1" ? "1" : "0";

    if (!uci_core.available() ||
        !uci_core.set("network.@globals[0].packet_steering", target_mode) ||
        !uci_core.commit("network"))
        action_fail("packet_steering", action, "Failed to update packet steering settings", current_mode, target_mode);

    command_status_from_args([ "/etc/init.d/network", "reload" ]);
    remove_file(SYSTEM_INFO_CACHE_FILE);
    action_success("packet_steering", action,
        target_mode == "1" ? "Packet Steering has been enabled" : "Packet Steering has been restored to default",
...
function set_direct_proxy(action) {
    ...
    if (!file_exists(SERVICE_INIT))
        action_fail("direct_proxy", action, "P99 service is not available", current_enabled, target_enabled);
...
function component_action(component, action) {
    ...
    capture_p99_running_state();

    if (component == "p99" && action == "check_update")
        check_p99();
    else if (component == "p99" && action == "install")
        install_p99();
...
else if (mode == "latest-p99-release-json")
    print(latest_p99_release_json());
else if (mode == "latest-p99-version")
    print(latest_p99_version(), "\n");
else if (mode == "p99-release-metadata")
    print(fetch_p99_latest_release_metadata(), "\n");
else {
    warn("Usage: components/action.uc <component-action|latest-p99-version|p99-release-metadata> ...\n");
    exit(1);
}
