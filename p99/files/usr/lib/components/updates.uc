...
const LIST_UPDATE_STATE_FILE = getenv("P99_LIST_UPDATE_STATE_FILE") || RUNTIME_STATE_DIR + "/list-update.timestamp";
const LIST_UPDATE_PID_FILE = getenv("P99_LIST_UPDATE_PID_FILE") || "/var/run/p99_list_update.pid";
const SUBSCRIPTION_UPDATE_STATE_DIR = getenv("P99_SUBSCRIPTION_UPDATE_STATE_DIR") || RUNTIME_STATE_DIR + "/subscription-update";
const SUBSCRIPTION_JOB_DIR = getenv("P99_SUBSCRIPTION_UPDATE_JOB_DIR") || "/var/run/p99/subscription-update-jobs";
...
const RUNTIME_CACHE_FORMAT = getenv("P99_RUNTIME_CACHE_FORMAT") || "10";
...
const RELOAD_STATE_FORMAT = getenv("P99_RELOAD_STATE_FORMAT") || "1";
...
const COMPONENT_UPDATE_CHECK_LOCK_DIR = getenv("P99_COMPONENT_UPDATE_CHECK_LOCK_DIR") || RUNTIME_STATE_DIR + "/component-update-check.lock";
...
const COMPONENT_JOB_STALE_GRACE_SECONDS = getenv("UPDATES_JOB_STALE_GRACE_SECONDS") || getenv("P99_UI_ACTION_STALE_GRACE_SECONDS") || "15";
const SUBSCRIPTION_JOB_FINISHED_TTL_MINUTES = getenv("P99_SUBSCRIPTION_UPDATE_JOB_FINISHED_TTL_MINUTES") || "60";
const SUBSCRIPTION_JOB_ORPHAN_OUTPUT_TTL_MINUTES = getenv("P99_SUBSCRIPTION_UPDATE_JOB_ORPHAN_OUTPUT_TTL_MINUTES") || "60";
const SUBSCRIPTION_JOB_STALE_GRACE_SECONDS = getenv("P99_UI_ACTION_STALE_GRACE_SECONDS") || "15";
const NFT_TABLE_NAME = getenv("NFT_TABLE_NAME") || "P99Table";
const NFT_COMMON_SET_NAME = getenv("NFT_COMMON_SET_NAME") || "p99_subnets";
const NFT_COMMON6_SET_NAME = getenv("NFT_COMMON6_SET_NAME") || "p99_subnets6";
const NFT_IP_PORT_SET_NAME = getenv("NFT_IP_PORT_SET_NAME") || "p99_ip_ports";
const NFT_IP_PORT6_SET_NAME = getenv("NFT_IP_PORT6_SET_NAME") || "p99_ip6_ports";
const NFT_DISCORD_SET_NAME = getenv("NFT_DISCORD_SET_NAME") || "p99_discord_subnets";
const NFT_DISCORD6_SET_NAME = getenv("NFT_DISCORD6_SET_NAME") || "p99_discord_subnets6";
const NFT_INTERFACE_SET_NAME = getenv("NFT_INTERFACE_SET_NAME") || "p99_interfaces";
...
const P99_MIRROR_BASE_URL = core_constants.P99_MIRROR_BASE_URL || "";
...
function ensure_subscription_runtime_dirs() {
    let command = command_env({
        ...
        P99_RUNTIME_CACHE_FORMAT: RUNTIME_CACHE_FORMAT,
...
function subscription_worker_env() {
    return {
        ...
        P99_RUNTIME_STATE_DIR: RUNTIME_STATE_DIR,
        P99_SUBSCRIPTION_UPDATE_STATE_DIR: SUBSCRIPTION_UPDATE_STATE_DIR,
        P99_SUBSCRIPTION_UPDATE_JOB_DIR: SUBSCRIPTION_JOB_DIR,
        P99_SUBSCRIPTION_LINKS_DIR: SUBSCRIPTION_LINKS_DIR,
        P99_SUBSCRIPTION_METADATA_DIR: SUBSCRIPTION_METADATA_DIR,
        P99_OUTBOUND_METADATA_DIR: OUTBOUND_METADATA_DIR,
        P99_SECTION_CACHE_DIR: SECTION_CACHE_DIR,
        ...
        P99_PERSISTENT_SUBSCRIPTION_CACHE_FORMAT_FILE: PERSISTENT_SUBSCRIPTION_CACHE_FORMAT_FILE,
        P99_PERSISTENT_SUBSCRIPTION_CACHE_FORMAT: PERSISTENT_SUBSCRIPTION_CACHE_FORMAT,
        P99_SUBSCRIPTION_UPDATE_JOB_FINISHED_TTL_MINUTES: SUBSCRIPTION_JOB_FINISHED_TTL_MINUTES,
        P99_SUBSCRIPTION_UPDATE_JOB_ORPHAN_OUTPUT_TTL_MINUTES: SUBSCRIPTION_JOB_ORPHAN_OUTPUT_TTL_MINUTES,
        P99_UI_ACTION_STALE_GRACE_SECONDS: SUBSCRIPTION_JOB_STALE_GRACE_SECONDS
    };
}
...
function valid_component_name(component) {
    ...
    return component == "p99" || component == "sing_box" || component == "zapret" ||
...
function component_update_check_cache() {
    ...
    if (enabled) {
        for (let component in [ "p99", "sing_box", "zapret", "zapret2", "byedpi" ]) {
...
function component_worker_env() {
    return {
        ...
        P99_UI_COMPONENT_ACTION_TRACKED: "1"
    };
}
...
function jsdelivr_fallback_url(url) {
    ...
    let sources = [
        ...
        [ P99_MIRROR_BASE_URL + "/p99/lists/allow-domains/", "https://cdn.jsdelivr.net/gh/itdoginfo/allow-domains@main/" ],
        [ "https://raw.githubusercontent.com/Greeg0ry/b4geoip-p99/main/", "https://cdn.jsdelivr.net/gh/Greeg0ry/b4geoip-p99@main/" ]
    ];
...
function github_raw_fallback_url(url) {
    url = as_string(url);
    let mirror_prefix = P99_MIRROR_BASE_URL != "" ?
        P99_MIRROR_BASE_URL + "/p99/lists/allow-domains/" : "";
...
function mirror_source_prefix(url) {
    let prefix = P99_MIRROR_BASE_URL != "" ?
        P99_MIRROR_BASE_URL + "/p99/lists/allow-domains/" : "";
...
function list_update() {
    ...
    if (!acquire_runtime_lock(RELOAD_LOCK_DIR, true)) {
        log_message("Lists update skipped because P99 reload did not release the runtime lock", "warn");
        list_update_pid_end();
        exit(1);
    }
...
function subscription_update_common(force, target_section, target_source_index) {
    ...
    if (!acquire_runtime_lock(RELOAD_LOCK_DIR, force)) {
        ...
        log_message("P99 reload is already running; skipping subscription update", "info");
        if (force)
