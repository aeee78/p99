...
const P99_INTERNAL_CONFIG_TRIGGER_GUARD = getenv("P99_INTERNAL_CONFIG_TRIGGER_GUARD") || "/var/run/p99.internal-config-change";
...
function subscription_cache_paths(section) {
    return [
        ...
        P99_SUBSCRIPTION_METADATA_DIR + "/" + section + ".json",
        P99_SUBSCRIPTION_LINKS_DIR + "/" + section + ".json",
        P99_OUTBOUND_METADATA_DIR + "/" + section + ".json",
        P99_SECTION_CACHE_DIR + "/" + section + ".json",
...
// Podkop Plus -> P99 source migration.
...
function migrate_p99_model(model) {
    let ctx = migration_context(model);
    apply_migrations(ctx);
    return ctx;
}
...
function ensure_runtime_cache_format() {
    ...
    if (first_line(P99_RUNTIME_CACHE_FORMAT_FILE) != P99_RUNTIME_CACHE_FORMAT) {
        if (first_line(P99_PERSISTENT_SUBSCRIPTION_CACHE_FORMAT_FILE) == P99_PERSISTENT_SUBSCRIPTION_CACHE_FORMAT)
            subscription_share_link.populate_subscription_dir(P99_PERSISTENT_SUBSCRIPTION_CACHE_DIR);
        clear_subscription_runtime_cache();
        ensure_runtime_dirs();
        fs.writefile(P99_RUNTIME_CACHE_FORMAT_FILE, P99_RUNTIME_CACHE_FORMAT + "\n");
    }

    if (first_line(P99_PERSISTENT_SUBSCRIPTION_CACHE_FORMAT_FILE) != P99_PERSISTENT_SUBSCRIPTION_CACHE_FORMAT) {
        run("rm -rf " + shell_quote(P99_PERSISTENT_SUBSCRIPTION_CACHE_DIR));
        ensure_dir(P99_PERSISTENT_SUBSCRIPTION_CACHE_DIR);
        run("chmod 700 " + shell_quote(P99_PERSISTENT_SUBSCRIPTION_CACHE_DIR) + " >/dev/null 2>&1");
        fs.writefile(P99_PERSISTENT_SUBSCRIPTION_CACHE_FORMAT_FILE, P99_PERSISTENT_SUBSCRIPTION_CACHE_FORMAT + "\n");
        run("chmod 600 " + shell_quote(P99_PERSISTENT_SUBSCRIPTION_CACHE_FORMAT_FILE) + " >/dev/null 2>&1");
    }
}

function remove_legacy_server_country_cache() {
    fs.unlink(P99_RUNTIME_STATE_DIR + "/server-country-cache.json");
}
...
function mark_internal_config_guard() {
    ...
    if (hash == "") {
        fs.unlink(P99_INTERNAL_CONFIG_TRIGGER_GUARD);
        return;
    }

    let stamp = clock();
    let tmp_path = P99_INTERNAL_CONFIG_TRIGGER_GUARD + "." + stamp[0] + "." + stamp[1];
    fs.writefile(tmp_path, as_string(stamp[0]) + "\n" + hash + "\n");
    if (!fs.rename(tmp_path, P99_INTERNAL_CONFIG_TRIGGER_GUARD))
        fs.unlink(tmp_path);
}
...
function main(argv) {
    ...
    warn("       config/migration.uc migrate-fixture <fixture.json> [p99|podkop]\n");
    return 1;
}
...
function module_exports() {
    return {
        ...
        migrate_p99_model: migrate_p99_model,
