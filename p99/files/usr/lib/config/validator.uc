...
function log_message(message, level) {
    ...
    run_args([ "logger", "-t", "p99", "[" + level + "] " + as_string(message) ]);
}
...
function safe_rm_rf(path) {
    path = as_string(path);
    if (path == "" || substr(path, 0, length("/var/run/p99/")) != "/var/run/p99/")
        return true;
...
function validate_runtime_mark_ranges_context(context) {
    ...
    if (fakeip_mark == null || outbound_mark == null)
        fail_validation("P99 marks contain invalid numeric constants. Aborted.");
...
function managed_sing_box_service_script(marker) {
    ...
    if (marker == "")
        marker = "P99 managed sing-box service for binary variants";
...
function install_managed_sing_box_service_script(ctx) {
    let stamp = clock();
    let tmp_file = sprintf("/etc/init.d/sing-box.p99.%d.%d", stamp[0], stamp[1]);
...
function check_provider_requirements(ctx) {
    ...
    check_provider_requirement(
        ...
        "Failed to prepare the P99 zapret state directory in " + ctx.zapret_state_dir + ". Aborted."
    );
    ...
    check_provider_requirement(
        ...
        "Failed to prepare the P99 zapret2 state directory in " + ctx.zapret2_state_dir + ". Aborted."
    );
    ...
    check_provider_requirement(
        ...
        "Failed to prepare the P99 ByeDPI state directory in " + ctx.byedpi_state_dir + ". Aborted."
    );
}
...
function check_runtime_requirements() {
    ...
    if (!command_exists("nft"))
        fail_requirement("Required nftables executable 'nft' is missing. Install package 'nftables-json' and start P99 again. Aborted.", "error");
    ...
    if (dhcp_has_https_dns_proxy_options("/etc/config/dhcp") === true)
        log_message("https-dns-proxy is enabled in DHCP config. Disable it or edit /etc/config/dhcp before starting P99.", "error");

    if (has_outbound_section(ctx))
        log_message("Proxy outbound configuration found", "debug");
    else
        log_message("No proxy outbound sections found. P99 will use direct and/or provider-only routing.", "warn");
