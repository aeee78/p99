...
function configured_mirror_base_url() {
    ...
    try {
        ...
        cursor.load("p99");
        let value = as_string(cursor.get("p99", "settings", "mirror_base_url"));
...
function constants_map() {
    ...
    c.P99_VERSION = env("P99_VERSION", "__COMPILED_VERSION_VARIABLE__");
    c.P99_CONFIG_NAME = env("P99_CONFIG_NAME", "p99");
    c.P99_CONFIG = env("P99_CONFIG", "/etc/config/" + c.P99_CONFIG_NAME);
    c.P99_BIN = env("P99_BIN", "/usr/bin/p99");
    c.P99_SERVICE_NAME = env("P99_SERVICE_NAME", "p99");
    c.P99_SERVICE_INIT = env("P99_SERVICE_INIT", "/etc/init.d/p99");
    c.P99_RELEASE_REPO = env("P99_RELEASE_REPO", "aeee78/p99");
    c.P99_LUCI_VIEW_NAMESPACE = env("P99_LUCI_VIEW_NAMESPACE", "p99");
    c.P99_LUCI_VIEW_DIR = env("P99_LUCI_VIEW_DIR", "/www/luci-static/resources/view/" + c.P99_LUCI_VIEW_NAMESPACE);
    c.P99_LUCI_I18N_DOMAIN = env("P99_LUCI_I18N_DOMAIN", "p99");
    ...
    c.RT_TABLE_NAME = env("RT_TABLE_NAME", "p99");

    c.NFT_TABLE_NAME = env("NFT_TABLE_NAME", "P99Table");
    c.NFT_LOCALV4_SET_NAME = env("NFT_LOCALV4_SET_NAME", "localv4");
    c.NFT_LOCALV6_SET_NAME = env("NFT_LOCALV6_SET_NAME", "localv6");
    c.NFT_COMMON_SET_NAME = env("NFT_COMMON_SET_NAME", "p99_subnets");
    c.NFT_COMMON6_SET_NAME = env("NFT_COMMON6_SET_NAME", "p99_subnets6");
    c.NFT_PORT_SET_NAME = env("NFT_PORT_SET_NAME", "p99_ports");
    c.NFT_IP_PORT_SET_NAME = env("NFT_IP_PORT_SET_NAME", "p99_ip_ports");
    c.NFT_IP_PORT6_SET_NAME = env("NFT_IP_PORT6_SET_NAME", "p99_ip6_ports");
    c.NFT_DISCORD_SET_NAME = env("NFT_DISCORD_SET_NAME", "p99_discord_subnets");
    c.NFT_DISCORD6_SET_NAME = env("NFT_DISCORD6_SET_NAME", "p99_discord_subnets6");
    c.NFT_INTERFACE_SET_NAME = env("NFT_INTERFACE_SET_NAME", "p99_interfaces");
    ...
    c.SB_MANAGED_SERVICE_MARKER = env("SB_MANAGED_SERVICE_MARKER", "P99 managed sing-box service for binary variants");
    ...
    c.SB_VARIANT_STATE_FILE = env("SB_VARIANT_STATE_FILE", "/etc/p99/sing-box-variant");
    c.SB_VERSION_STATE_FILE = env("SB_VERSION_STATE_FILE", "/etc/p99/sing-box-version");

    c.P99_MIRROR_BASE_URL = env("P99_MIRROR_BASE_URL", configured_mirror_base_url());
    let mirror_lists = c.P99_MIRROR_BASE_URL != "" ? c.P99_MIRROR_BASE_URL + "/p99/lists" : "";
    ...
    c.ZAPRET_LEGACY_RUNTIME_BASE_DIR = env("ZAPRET_LEGACY_RUNTIME_BASE_DIR", "/var/run/p99/zapret-runtime");
    c.ZAPRET_NFQWS_BIN = env("ZAPRET_NFQWS_BIN", c.ZAPRET_PROVIDER_NFQWS_BIN);
    c.ZAPRET_STATE_DIR = env("ZAPRET_STATE_DIR", "/var/run/p99/zapret");
    ...
    c.ZAPRET2_STATE_DIR = env("ZAPRET2_STATE_DIR", "/var/run/p99/zapret2");
    ...
    c.BYEDPI_STATE_DIR = env("BYEDPI_STATE_DIR", "/var/run/p99/byedpi");
