...
const P99_CONFIG = getenv("P99_CONFIG") || constants.P99_CONFIG || "/etc/config/" + CONFIG_NAME;
const P99_SERVICE_NAME = getenv("P99_SERVICE_NAME") || constants.P99_SERVICE_NAME || "p99";
const P99_RELEASE_REPO = getenv("P99_RELEASE_REPO") || constants.P99_RELEASE_REPO || "aeee78/p99";
const P99_LUCI_VIEW_DIR = getenv("P99_LUCI_VIEW_DIR") || constants.P99_LUCI_VIEW_DIR || "/www/luci-static/resources/view/p99";
...
const SYSTEM_INFO_CACHE_TTL = int(getenv("P99_SYSTEM_INFO_CACHE_TTL") || "3600");
...
const RT_TABLE_NAME = getenv("RT_TABLE_NAME") || constants.RT_TABLE_NAME || "p99";
const NFT_TABLE_NAME = getenv("NFT_TABLE_NAME") || constants.NFT_TABLE_NAME || "P99Table";
const NFT_FAKEIP_MARK = getenv("NFT_FAKEIP_MARK") || constants.NFT_FAKEIP_MARK || "0x04000000";
const NFT_COMMON_SET_NAME = getenv("NFT_COMMON_SET_NAME") || constants.NFT_COMMON_SET_NAME || "p99_subnets";
const NFT_PORT_SET_NAME = getenv("NFT_PORT_SET_NAME") || constants.NFT_PORT_SET_NAME || "p99_ports";
const NFT_IP_PORT_SET_NAME = getenv("NFT_IP_PORT_SET_NAME") || constants.NFT_IP_PORT_SET_NAME || "p99_ip_ports";
const NFT_INTERFACE_SET_NAME = getenv("NFT_INTERFACE_SET_NAME") || constants.NFT_INTERFACE_SET_NAME || "p99_interfaces";
const NFT_DISCORD_SET_NAME = getenv("NFT_DISCORD_SET_NAME") || constants.NFT_DISCORD_SET_NAME || "p99_discord_subnets";
...
const SB_VARIANT_STATE_FILE = getenv("SB_VARIANT_STATE_FILE") || constants.SB_VARIANT_STATE_FILE || "/etc/p99/sing-box-variant";
const SING_BOX_BIN_PATH = getenv("P99_DIAGNOSTICS_SING_BOX_BIN_PATH") || "/usr/bin/sing-box";
...
function check_logs() {
    ...
    let rendered = status_capture([ "p99-logs" ], command_output_from_args([ "logread" ]));
    if (rendered.output != "")
...
function p99_logs_fixture() {
    let rendered = status_capture([ "p99-logs" ], read_stdin());
    if (rendered.output != "")
        print(rendered.output);
...
function show_config(visibility) {
    visibility = as_string(visibility || "masked");
    if (!file_exists(P99_CONFIG)) {
        nolog("Configuration file not found");
        return 1;
    }
    if (visibility == "raw")
        print(as_string(fs.readfile(P99_CONFIG)));
    else
        print(status_output([ "p99-config-masked", P99_CONFIG ], null));
    return 0;
}
...
function get_luci_app_version() {
    let path = P99_LUCI_VIEW_DIR + "/main.js";
    ...
    for (let line in split(as_string(data), "\n")) {
        ...
        if (matched != null && matched[1] == "P99_LUCI_APP_VERSION")
...
function system_info_cache_is_valid() {
    ...
    if (now > 0 && generated_at > 0 && SYSTEM_INFO_CACHE_TTL > 0 && now - generated_at >= SYSTEM_INFO_CACHE_TTL)
        return false;
    if (cache.p99_version != P99_VERSION || cache.luci_app_version != get_luci_app_version())
        return false;
...
function build_system_info() {
    let p99_latest_version = first_line_value("/tmp/p99.latest-version.cache", "unknown");
    ...
    return {
        ...
        p99_latest_version: p99_latest_version || "unknown",
        luci_app_version,
        sing_box_version,
...
function dnsmasq_has_p99_dns() {
    return module_success(DNS_APPLY_UC, [ "has-p99-dns" ]);
}
...
function get_sing_box_status() {
    ...
    let dns_configured = dnsmasq_has_p99_dns() ? 1 : 0;
...
function get_status() {
    let running = module_success(SERVICE_STATE_UC, [
        "p99-stably-running", RT_TABLE_NAME, NFT_TABLE_NAME, NFT_FAKEIP_MARK, RUNTIME_STABLE_MIN_AGE
    ]) ? 1 : 0;
    let enabled = file_executable("/etc/rc.d/S99" + P99_SERVICE_NAME) ? 1 : 0;
    let dns_configured = dnsmasq_has_p99_dns() ? 1 : 0;
    write_service_status(running, enabled, dns_configured);
    return 0;
}
...
function automatic_latency_test() {
    ...
    if (length(proxy_tags) == 0) {
        ...
        command_success_from_args([ "logger", "-t", "p99", "[info] Automatic latency test skipped: no proxy outbounds available" ]);
        return 0;
    }
...
function global_check(arg1, arg2) {
    ...
    if (file_executable("/etc/init.d/zapret") && command_success_from_args([ "/etc/init.d/zapret", "status" ])) {
        print_global("━━━━━━━━━━━━━━━━━━━━━━━━━━━");
        print_global("⚠️ Standalone zapret service is active. P99 uses separate queues, but packet-level policy overlap is possible.");
    }
    else if (file_executable("/etc/init.d/zapret") && command_success_from_args([ "/etc/init.d/zapret", "enabled" ])) {
        print_global("━━━━━━━━━━━━━━━━━━━━━━━━━━━");
        print_global("⚠️ Standalone zapret autostart is enabled. P99 will not modify /etc/config/zapret.");
    }

    if (file_executable("/etc/init.d/zapret2") && command_success_from_args([ "/etc/init.d/zapret2", "status" ])) {
        print_global("━━━━━━━━━━━━━━━━━━━━━━━━━━━");
        print_global("⚠️ Standalone zapret2 service is active. P99 uses separate queues, but packet-level policy overlap is possible.");
    }
    else if (file_executable("/etc/init.d/zapret2") && command_success_from_args([ "/etc/init.d/zapret2", "enabled" ])) {
        print_global("━━━━━━━━━━━━━━━━━━━━━━━━━━━");
        print_global("⚠️ Standalone zapret2 autostart is enabled. P99 will not modify /etc/config/zapret2.");
    }
...
else if (mode == "p99-logs-fixture")
    exit(p99_logs_fixture());
