...
function p99_config_masked_line(line) {
...
function p99_config_masked(path) {
    let data = fs.readfile(path);
    if (data == null)
        exit(1);
    ...
    for (let i = 0; i < length(lines); i++) {
        ...
        let masked = p99_config_masked_line(line);
        print(masked, "\n");
...
function render_global_system_info() {
    let value = object_or_empty(read_stdin_json());
    let p99_version = object_value(value, "p99_version") || "unknown";
...
function nft_ruleset_other_mark_lines(table_name) {
    ...
    for (let line in split(read_stdin(), "\n")) {
        ...
        if (!in_p99_table && (index(line, "mark set") >= 0 || index(line, "meta mark") >= 0))
            print_line(line);
    }
}
        ...
        if (line_contains(line, needle))
        {
            if (needle == "sing-box" && is_udp_unsupported_outbound_log(line)) {
                if (!udp_http_notice_emitted) {
                    push(filtered, "UDP traffic through HTTP outbounds is not supported by sing-box; repeated UDP warnings for HTTP outbounds are hidden by P99.");
...
function render_p99_logs() {
    ...
    for (let line in lines) {
        if (line == "")
            continue;
        if (!line_contains(line, "p99") && !line_contains(line, "sing-box"))
            continue;
        ...
        if (line_contains(line, "p99") && line_contains(line, "Starting P99"))
    ...
    print_line("No 'Starting P99' message found, showing last 100 lines");
...
function system_info_cache_valid(path, p99_version, luci_app_version, ttl, now) {
    ...
    return cache.p99_version == p99_version && cache.luci_app_version == luci_app_version;
}

function system_info_json() {
    write_json({
        p99_version: as_string(ARGV[1]),
        p99_latest_version: as_string(ARGV[2]),
...
else if (mode == "p99-config-masked")
    p99_config_masked(ARGV[1]);
