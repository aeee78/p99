...
ROOT="${P99_UNINSTALL_ROOT:-}"
if [ -n "$ROOT" ]; then ROOT="$(cd "$ROOT" && pwd -P)"; fi
MIRROR="${P99_MIRROR_BASE_URL:-}"
if [ -z "$MIRROR" ]; then MIRROR="$(uci -q get p99.settings.mirror_base_url 2>/dev/null || true)"; fi
MIRROR="${MIRROR:-https://mirror.51343.ru}"
BIN="$ROOT/usr/bin/p99"
LOCK="$ROOT/tmp/p99-full-uninstall.lock"
COMPONENT_LOCK="$ROOT/var/run/p99/component-action.lock"
PACKAGES="luci-i18n-p99-ru luci-app-p99 p99 sing-box sing-box-tiny sing-box-extended"
PHASE=preflight
...
repository_plan() {
    ...
    for file in "$ROOT/etc/opkg/distfeeds.conf" "$ROOT/etc/opkg/customfeeds.conf" \
        ...
        [ -f "$file" ] || continue
        [ "$file" != "$ROOT/etc/apk/repositories.d/p99.list" ] || continue
        source="${file}.pre-p99-mirror"
...
run() {
    ...
    PHASE=stop
    state running
    if [ -x "$ROOT/etc/init.d/p99" ]; then
        "$ROOT/etc/init.d/p99" stop
        "$ROOT/etc/init.d/p99" disable
    fi
    if [ -x "$ROOT/etc/init.d/p99-torrserver-direct" ]; then
        "$ROOT/etc/init.d/p99-torrserver-direct" stop
        "$ROOT/etc/init.d/p99-torrserver-direct" disable
    fi
    ...
    while IFS='|' read -r file source; do
        cp "$source" "$file.p99-restore"
        chmod 644 "$file.p99-restore"
        mv "$file.p99-restore" "$file"
    done < "$JOB/repositories"
    rm -f "$ROOT/etc/apk/repositories.d/p99.list" "$ROOT/etc/apk/keys/p99-mirror.pem"
    ...
    for directory in /etc/p99 /etc/sing-box /tmp/sing-box /usr/lib/p99 \
        /usr/share/p99 /www/luci-static/resources/view/p99; do
        rm -rf "$ROOT$directory"
    done
    for file in /etc/config/p99 /etc/config/sing-box /etc/config/p99-opkg \
        /etc/config/sing-box-opkg /usr/bin/p99 /usr/bin/sing-box /usr/lib/libcronet.so \
        /etc/init.d/p99 /etc/init.d/sing-box /etc/init.d/p99-torrserver-direct /etc/uci-defaults/50_luci-p99 \
        /usr/share/luci/menu.d/luci-app-p99.json /usr/share/rpcd/acl.d/luci-app-p99.json; do
        rm -f "$ROOT$file"
    done
    for file in "$ROOT"/usr/lib/lua/luci/i18n/p99.* \
        "$ROOT"/tmp/luci-indexcache* "$ROOT"/tmp/luci-modulecache/*; do
        [ ! -f "$file" ] || rm -f "$file"
    done
    while IFS='|' read -r file source; do
        rm -f "${file}.pre-p99-mirror"
    done < "$JOB/repositories"
    # Leave the component lock intact until finish() releases it.
    for item in "$ROOT"/var/run/p99/*; do
...
case "${1:-}" in
    start)
        mkdir -p "$ROOT/tmp" "$ROOT/www" "$ROOT/var/run/p99"
        ...
        printf '%s\n' "$$" > "$LOCK/pid"
        printf '%s\n' "$$" > "$COMPONENT_LOCK/pid"
        JOB="$(mktemp -d "$ROOT/tmp/p99-uninstall.XXXXXX")"
