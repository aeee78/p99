...
const START_RETRY_FILE = getenv("P99_START_RETRY_FILE") || RUNTIME_STATE_DIR + "/start.retry";
const START_RETRY_PID_FILE = getenv("P99_START_RETRY_PID_FILE") || RUNTIME_STATE_DIR + "/start-retry.pid";
const START_FAILURE_FILE = getenv("P99_START_FAILURE_FILE") || RUNTIME_STATE_DIR + "/start.failure";
const START_RETRY_DELAY_SECONDS = getenv("P99_START_RETRY_DELAY_SECONDS") || "30";
...
const CONFIG_CHANGE_REASON = getenv("P99_CONFIG_CHANGE_REASON") || "on_config_change";
...
function retry_start_on_wan_up(owner_pid) {
    ...
    command_success_from_args([ "logger", "-t", SERVICE_NAME, "[info] Retrying failed P99 start after WAN came up" ]);
...
function handle_wan_up(owner_pid) {
    ...
    if (action == "reload") {
        ...
        command_success_from_args([ "logger", "-t", SERVICE_NAME, "[info] Reloading P99 after monitored WAN came up" ]);
...
function ui_action_tracked() {
    return as_string(getenv("P99_UI_ACTION_TRACKED") || "0") == "1";
}
...
function start_service(reason, owner_pid) {
    ...
    if (status == 0) {
        ...
        clear_start_retry(START_RETRY_FILE);
        cancel_scheduled_start_retry(START_RETRY_PID_FILE);
        command_success_from_args([ "logger", "-t", SERVICE_NAME, "[error] P99 startup retry suppressed because all rule-set download sources failed; see the fatal startup error in LuCI logs" ]);
    }
    else {
        ...
        command_success_from_args([ "logger", "-t", SERVICE_NAME, "[warn] P99 start failed; scheduled an automatic retry" ]);
    }
...
function reload_service(reason, owner_pid) {
    ...
    let status = command_status(command_from_args([ "env", "P99_UI_ACTION_TRACKED=1", BIN_PATH, "reload", reason ]) + " >/dev/null 2>&1");
