...
const CONFIG_NAME = getenv("P99_CONFIG_NAME") || constant_value("P99_CONFIG_NAME", "p99");
const CONFIG_FILE = getenv("P99_CONFIG_FILE") || "/etc/config/" + CONFIG_NAME;
const LIB_DIR = getenv("P99_LIB") || "/usr/lib/p99";
const BIN_PATH = getenv("P99_BIN") || constant_value("P99_BIN", "/usr/bin/p99");
const SERVICE_INIT = getenv("P99_SERVICE_INIT") || constant_value("P99_SERVICE_INIT", "/etc/init.d/p99");
const SERVICE_NAME = getenv("P99_SERVICE_NAME") || constant_value("P99_SERVICE_NAME", "p99");
const LUCI_VIEW_DIR = getenv("P99_LUCI_VIEW_DIR") || constant_value("P99_LUCI_VIEW_DIR", "/www/luci-static/resources/view/p99");
const LUCI_I18N_DOMAIN = getenv("P99_LUCI_I18N_DOMAIN") || constant_value("P99_LUCI_I18N_DOMAIN", "p99");
...
const RELOAD_STATE_FILE = getenv("P99_RELOAD_STATE_FILE") || RUNTIME_STATE_DIR + "/reload-state";
const RELOAD_STATE_SNAPSHOT_FILE = getenv("P99_RELOAD_STATE_SNAPSHOT_FILE") || RUNTIME_STATE_DIR + "/reload-state.snapshot." + clock()[0] + "." + clock()[1];
const PENDING_RELOAD_FILE = getenv("P99_PENDING_RELOAD_FILE") || RUNTIME_STATE_DIR + "/reload.pending";
const START_FAILURE_FILE = getenv("P99_START_FAILURE_FILE") || RUNTIME_STATE_DIR + "/start.failure";
const SERVICE_TRIGGER_SYNC_FILE = getenv("P99_SERVICE_TRIGGER_SYNC_FILE") || RUNTIME_STATE_DIR + "/service-triggers.sync";
const SUBSCRIPTION_UPDATE_STATE_DIR = getenv("P99_SUBSCRIPTION_UPDATE_STATE_DIR") || RUNTIME_STATE_DIR + "/subscription-update";
const SUBSCRIPTION_LINKS_DIR = getenv("P99_SUBSCRIPTION_LINKS_DIR") || RUNTIME_STATE_DIR + "/subscription-links";
const SUBSCRIPTION_METADATA_DIR = getenv("P99_SUBSCRIPTION_METADATA_DIR") || RUNTIME_STATE_DIR + "/subscription-metadata";
const OUTBOUND_METADATA_DIR = getenv("P99_OUTBOUND_METADATA_DIR") || RUNTIME_STATE_DIR + "/outbound-metadata";
const SECTION_CACHE_DIR = getenv("P99_SECTION_CACHE_DIR") || RUNTIME_STATE_DIR + "/section-cache";
const RULE_CONDITION_CACHE_DIR = getenv("P99_RULE_CONDITION_CACHE_DIR") || RUNTIME_STATE_DIR + "/rule-condition-cache";
const RUNTIME_CACHE_FORMAT_FILE = getenv("P99_RUNTIME_CACHE_FORMAT_FILE") || RUNTIME_STATE_DIR + "/cache-format";
const PERSISTENT_SUBSCRIPTION_CACHE_DIR = getenv("P99_PERSISTENT_SUBSCRIPTION_CACHE_DIR") || "/etc/p99/subscription-cache";
const PERSISTENT_SUBSCRIPTION_CACHE_FORMAT_FILE = getenv("P99_PERSISTENT_SUBSCRIPTION_CACHE_FORMAT_FILE") || PERSISTENT_SUBSCRIPTION_CACHE_DIR + "/cache-format";
const PERSISTENT_SUBSCRIPTION_CACHE_FORMAT = getenv("P99_PERSISTENT_SUBSCRIPTION_CACHE_FORMAT") || "10";
const LIFECYCLE_UC = LIB_DIR + "/service/lifecycle.uc";
const SUBSCRIPTION_BOOTSTRAP_RETRY_PID_FILE = getenv("P99_SUBSCRIPTION_BOOTSTRAP_RETRY_PID_FILE") || RUNTIME_STATE_DIR + "/subscription-bootstrap-retry.pid";
const DNS_FAILOVER_STATE_FILE = getenv("P99_DNS_FAILOVER_STATE_FILE") || RUNTIME_STATE_DIR + "/dns-failover.json";
const DNS_FAILOVER_PID_FILE = getenv("P99_DNS_FAILOVER_PID_FILE") || RUNTIME_STATE_DIR + "/dns-failover.pid";
const SUBSCRIPTION_UPDATE_LOCK_DIR = getenv("P99_SUBSCRIPTION_UPDATE_LOCK_DIR") || RUNTIME_STATE_DIR + "/subscription-update.lock";
const RELOAD_LOCK_DIR = getenv("P99_RELOAD_LOCK_DIR") || "/var/run/p99.reload.lock";
const INTERNAL_CONFIG_TRIGGER_GUARD = getenv("P99_INTERNAL_CONFIG_TRIGGER_GUARD") || "/var/run/p99.internal-config-change";

const LIST_UPDATE_CRON_MARKER = getenv("P99_LIST_UPDATE_CRON_MARKER") || "# p99-list-update";
const SUBSCRIPTION_UPDATE_CRON_MARKER = getenv("P99_SUBSCRIPTION_UPDATE_CRON_MARKER") || "# p99-subscription-update";
const COMPONENT_UPDATE_CHECK_CRON_MARKER = getenv("P99_COMPONENT_UPDATE_CHECK_CRON_MARKER") || "# p99-component-update-check";
const RELOAD_STATE_FORMAT = int(getenv("P99_RELOAD_STATE_FORMAT") || "1");
const RUNTIME_CACHE_FORMAT = int(getenv("P99_RUNTIME_CACHE_FORMAT") || "10");
const RUNTIME_STABLE_MIN_AGE = int(getenv("P99_RUNTIME_STABLE_MIN_AGE") || "2");
const SING_BOX_START_STABLE_MIN_AGE = int(getenv("P99_SING_BOX_START_STABLE_MIN_AGE") || "8");
const SING_BOX_START_VERIFY_TIMEOUT = int(getenv("P99_SING_BOX_START_VERIFY_TIMEOUT") || "10");
const NFT_POPULATE_ENABLED_DEFAULT = int(getenv("P99_NFT_POPULATE_ENABLED") || "1");
...
const RT_TABLE_NAME = constant_value("RT_TABLE_NAME", "p99");
const NFT_TABLE_NAME = constant_value("NFT_TABLE_NAME", "P99Table");
...
const NFT_COMMON_SET_NAME = constant_value("NFT_COMMON_SET_NAME", "p99_subnets");
const NFT_COMMON6_SET_NAME = constant_value("NFT_COMMON6_SET_NAME", "p99_subnets6");
const NFT_PORT_SET_NAME = constant_value("NFT_PORT_SET_NAME", "p99_ports");
const NFT_IP_PORT_SET_NAME = constant_value("NFT_IP_PORT_SET_NAME", "p99_ip_ports");
const NFT_IP_PORT6_SET_NAME = constant_value("NFT_IP_PORT6_SET_NAME", "p99_ip6_ports");
const NFT_INTERFACE_SET_NAME = constant_value("NFT_INTERFACE_SET_NAME", "p99_interfaces");
...
const SB_VARIANT_STATE_FILE = constant_value("SB_VARIANT_STATE_FILE", "/etc/p99/sing-box-variant");
const SB_VERSION_STATE_FILE = constant_value("SB_VERSION_STATE_FILE", "/etc/p99/sing-box-version");
...
let subscription_caches_prepared = getenv("P99_SUBSCRIPTION_CACHES_PREPARED") || "0";
let subscription_runtime_no_refresh = getenv("P99_SUBSCRIPTION_RUNTIME_NO_REFRESH") || "0";
...
function lifecycle_env() {
    let result = {
        P99_CONFIG_NAME: CONFIG_NAME,
        P99_LIB: LIB_DIR,
        P99_BIN: BIN_PATH,
        P99_SERVICE_INIT: SERVICE_INIT,
        P99_RUNTIME_STATE_DIR: RUNTIME_STATE_DIR,
        P99_SYSTEM_INFO_CACHE_FILE: SYSTEM_INFO_CACHE_FILE,
        P99_SUBSCRIPTION_UPDATE_STATE_DIR: SUBSCRIPTION_UPDATE_STATE_DIR,
        P99_SUBSCRIPTION_LINKS_DIR: SUBSCRIPTION_LINKS_DIR,
        P99_SUBSCRIPTION_METADATA_DIR: SUBSCRIPTION_METADATA_DIR,
        P99_OUTBOUND_METADATA_DIR: OUTBOUND_METADATA_DIR,
        P99_SECTION_CACHE_DIR: SECTION_CACHE_DIR,
        P99_RULE_CONDITION_CACHE_DIR: RULE_CONDITION_CACHE_DIR,
        P99_RUNTIME_CACHE_FORMAT_FILE: RUNTIME_CACHE_FORMAT_FILE,
        P99_RUNTIME_CACHE_FORMAT: as_string(RUNTIME_CACHE_FORMAT),
        P99_PERSISTENT_SUBSCRIPTION_CACHE_DIR: PERSISTENT_SUBSCRIPTION_CACHE_DIR,
        P99_PERSISTENT_SUBSCRIPTION_CACHE_FORMAT_FILE: PERSISTENT_SUBSCRIPTION_CACHE_FORMAT_FILE,
        P99_PERSISTENT_SUBSCRIPTION_CACHE_FORMAT: PERSISTENT_SUBSCRIPTION_CACHE_FORMAT,
        P99_SUBSCRIPTION_BOOTSTRAP_RETRY_PID_FILE: SUBSCRIPTION_BOOTSTRAP_RETRY_PID_FILE,
        P99_DNS_FAILOVER_STATE_FILE: DNS_FAILOVER_STATE_FILE,
        P99_DNS_FAILOVER_PID_FILE: DNS_FAILOVER_PID_FILE,
        P99_SUBSCRIPTION_UPDATE_LOCK_DIR: SUBSCRIPTION_UPDATE_LOCK_DIR,
        P99_PENDING_RELOAD_FILE: PENDING_RELOAD_FILE,
        P99_RELOAD_LOCK_DIR: RELOAD_LOCK_DIR,
        ...
        P99_RULE_CONDITION_CACHE_ENABLED: as_string(rule_condition_cache_enabled)
    };
...
function dnsmasq_has_p99_managed_state() {
...
function start_sing_box_and_wait() {
    // During rcS startup P99 inherits procd's service lock on fd 1000.
...
function refresh_rulesets_then_start_latency() {
    ...
    if (status == 0) {
        log_message("Rule-set cache changed; reloading P99 before automatic latency test", "info");
...
function start_impl() {
    ...
    else if (dnsmasq_has_p99_managed_state()) {
...
function cleanup_failed_runtime() {
    ...
    log_message("Cleaning up P99 runtime after failed start/reload", "info");
    ...
    if (status != 0)
        log_message("Failed to fully clean up P99 runtime after start/reload failure", "warn");
...
function start() {
    ...
    if (status != 0) {
        log_message("Startup verification failed after P99 was started; rolling back DNS changes", "warn");
        cleanup_failed_runtime();
        return status;
    }
...
function restart_runtime_for_reload() {
    ...
    log_message("Reload requires a full P99 runtime restart", "info");
    ...
    if (status != 0) {
        log_message("Reload runtime restart verification failed after P99 was started; rolling back DNS changes", "fatal");
        cleanup_failed_runtime();
        return status;
    }
...
function reload(reason) {
    ...
    log_message("Reloading P99", "info");
    ...
    if (!module_success(STATE_UC, [ "p99-running", RT_TABLE_NAME, NFT_TABLE_NAME, NFT_FAKEIP_MARK ])) {
        log_message("Runtime state is incomplete; restarting P99 runtime", "info");
    ...
    let dnsmasq_managed_state = dnsmasq_has_p99_managed_state() ? 1 : 0;
    ...
    if (as_string(getenv("P99_RUNTIME_CACHE_INVALIDATED") || "0") == "1" ||
    ...
    if (plan_result.status != 0) {
        if (plan_result.status == 2) {
            log_message("Reload state is unavailable; restarting P99 runtime", "info");
            return finish_reload_status(restart_runtime_for_reload(), reload_config_fingerprint);
        }
    ...
    if (plan.needs_sing_box_reload == 1) {
        ...
        if (status != 0) {
            log_message("Reload verification failed after sing-box was reloaded; stopping P99 runtime", "fatal");
            cleanup_failed_runtime();
            return status;
        }
        status = module_status(PRIORITY_UC, [ "start-runtime" ]);
    ...
    // fingerprint have been committed, otherwise P99 mistakes its own
...
function reload_tracked(reason) {
    if (as_string(getenv("P99_UI_ACTION_TRACKED") || "0") == "1")
...
function restart() {
    log_message("Restarting P99", "info");
    ...
    log_message("Restart verification failed after P99 was started; stopping P99 runtime", "fatal");
    cleanup_failed_runtime();
    return 1;
}
...
function uninstall() {
    log_message("Uninstalling P99", "info");
    ...
    if (fs.stat("/etc/init.d/p99") != null) {
        command_success_from_args([ "/etc/init.d/p99", "stop" ]);
        command_success_from_args([ "/etc/init.d/p99", "disable" ]);
    }

    package_manager_remove_if_installed("luci-i18n-p99-ru");
    package_manager_remove_if_installed("luci-app-p99");
    ...
    command_success_from_args([ "rm", "-rf", "/usr/lib/p99" ]);
    command_success_from_args([ "rm", "-rf", LUCI_VIEW_DIR ]);
    remove_file(SERVICE_INIT);
    remove_file(BIN_PATH);
    remove_file("/usr/share/luci/menu.d/luci-app-p99.json");
    remove_file("/usr/share/rpcd/acl.d/luci-app-p99.json");
    remove_file("/etc/uci-defaults/50_luci-p99");
