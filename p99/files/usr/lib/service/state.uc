...
const DEFAULT_PENDING_RELOAD_FILE = getenv("P99_PENDING_RELOAD_FILE") || "/var/run/p99/reload.pending";
const DEFAULT_SERVICE_INIT = getenv("P99_SERVICE_INIT") || "/etc/init.d/p99";
...
function run_pending_reload_if_requested(path, init_script) {
    ...
    command_success_from_args([ "logger", "-t", "p99", "[info] Applying pending P99 reload" ]);
...
function cleanup_rule_condition_cache(path) {
    ...
    system("rm -rf /tmp/p99-rule-cache.* >/dev/null 2>&1");
}
...
function hup_sing_box_runtime() {
    ...
    command_success_from_args([ "logger", "-t", "p99", "[info] Applying DNS failover with sing-box SIGHUP reload" ]);
...
function reload_sing_box_runtime(previous_pid, config_hash_before, config_hash_after, force) {
    if (!sing_box_runtime_reload_needed(config_hash_before, config_hash_after, force)) {
        command_success_from_args([ "logger", "-t", "p99", "[info] sing-box reload skipped: configuration is unchanged" ]);
        return;
    }

    previous_pid = sing_box_reload_previous_pid(previous_pid, config_hash_before, config_hash_after);
    command_success_from_args([ "logger", "-t", "p99", "[info] Reloading sing-box runtime" ]);
    if (!command_success_from_args([ "/etc/init.d/sing-box", "reload" ])) {
        command_success_from_args([ "logger", "-t", "p99", "[fatal] Failed to reload sing-box. Aborted." ]);
        exit(1);
    }

    let timeout = getenv("P99_SING_BOX_RELOAD_PID_TIMEOUT") || "15";
    if (previous_pid > 0 && !wait_sing_box_pid_replacement(previous_pid, timeout)) {
        command_success_from_args([ "logger", "-t", "p99", "[fatal] sing-box reload did not replace the running process. Aborted." ]);
        exit(1);
    }
}
...
function p99_runtime_network_configured(rt_table, nft_table, mark) {
...
function p99_running(rt_table, nft_table, mark) {
    return sing_box_service_running() && sing_box_runtime_ports_ready() &&
        p99_runtime_network_configured(rt_table, nft_table, mark);
}

function p99_stably_running(rt_table, nft_table, mark, min_age) {
    return sing_box_service_stable(min_age) && sing_box_runtime_ports_ready() &&
        p99_runtime_network_configured(rt_table, nft_table, mark);
}

function wait_p99_stable_start(rt_table, nft_table, mark, min_age, timeout) {
    timeout = int(timeout || 8);
    while (timeout > 0) {
        if (p99_stably_running(rt_table, nft_table, mark, min_age))
            return true;
    ...
    return p99_stably_running(rt_table, nft_table, mark, min_age);
}
...
function dnsmasq_signature_body(settings, dnsmasq, legacy_dnsmasq_present) {
    ...
    body = signature_add_value(body, "dhcp.@dnsmasq[0].p99_server", option(dnsmasq, "p99_server", ""));
    body = signature_add_value(body, "dhcp.@dnsmasq[0].p99_noresolv", option(dnsmasq, "p99_noresolv", ""));
    body = signature_add_value(body, "dhcp.@dnsmasq[0].p99_cachesize", option(dnsmasq, "p99_cachesize", ""));
    body = signature_add_value(body, "dhcp.p99.present", arg_bool(legacy_dnsmasq_present) ? "1" : "0");
...
function uci_dnsmasq() {
    return {
        ...
        p99_server: uci_get("dhcp.@dnsmasq[0].p99_server"),
        p99_noresolv: uci_get("dhcp.@dnsmasq[0].p99_noresolv"),
        p99_cachesize: uci_get("dhcp.@dnsmasq[0].p99_cachesize")
    };
}
...
if (mode == "file-first-line")
...
else if (mode == "p99-running")
    exit(p99_running(ARGV[1], ARGV[2], ARGV[3]) ? 0 : 1);
else if (mode == "p99-stably-running")
    exit(p99_stably_running(ARGV[1], ARGV[2], ARGV[3], ARGV[4]) ? 0 : 1);
else if (mode == "wait-p99-stable-start")
    exit(wait_p99_stable_start(ARGV[1], ARGV[2], ARGV[3], ARGV[4], ARGV[5]) ? 0 : 1);
    ...
    exit(print_signature_hash(dnsmasq_signature_body(uci_settings(), uci_dnsmasq(), uci_exists("dhcp.p99"))) ? 0 : 1);
