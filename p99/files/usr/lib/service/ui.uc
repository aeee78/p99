...
const SERVICE_NAME = getenv("P99_SERVICE_NAME") || "p99";
const STATE_UC = LIB_DIR + "/service/state.uc";
const UI_UC = LIB_DIR + "/service/ui.uc";
const STATE_DIR = getenv("P99_UI_STATE_DIR") || "/var/run/p99/ui-state";
const PENDING_RELOAD_FILE = getenv("P99_PENDING_RELOAD_FILE") || "/var/run/p99/reload.pending";
const SERVICE_ACTION_DIR = getenv("P99_UI_SERVICE_ACTION_DIR") || STATE_DIR + "/service-actions";
const SERVICE_ACTION_LOCK_DIR = getenv("P99_UI_SERVICE_ACTION_LOCK_DIR") || STATE_DIR + "/service-actions.lock";
const LATENCY_ACTION_DIR = getenv("P99_UI_LATENCY_ACTION_DIR") || STATE_DIR + "/latency-actions";
const LATENCY_TEST_LOCK_DIR = getenv("P99_LATENCY_TEST_LOCK_DIR") || "/var/run/p99/automatic-latency-test.lock";
const COMPONENT_ACTION_DIR = getenv("P99_UI_COMPONENT_ACTION_DIR") || getenv("UPDATES_JOB_DIR") || "/var/run/p99/component-actions";
const SUBSCRIPTION_ACTION_DIR = getenv("P99_UI_SUBSCRIPTION_ACTION_DIR") || getenv("P99_SUBSCRIPTION_UPDATE_JOB_DIR") || "/var/run/p99/subscription-update-jobs";
const SING_BOX_VERSION_CACHE_FILE = getenv("P99_UI_SING_BOX_VERSION_CACHE_FILE") || STATE_DIR + "/sing-box-version";
const SING_BOX_VERSION_CACHE_LOCK_DIR = getenv("P99_UI_SING_BOX_VERSION_CACHE_LOCK_DIR") || SING_BOX_VERSION_CACHE_FILE + ".lock";
const SING_BOX_VARIANT_STATE_FILE = getenv("P99_UI_SING_BOX_VARIANT_STATE_FILE") || "/etc/p99/sing-box-variant";
const SING_BOX_BIN_PATH = getenv("P99_UI_SING_BOX_BIN_PATH") || "/usr/bin/sing-box";
const SING_BOX_VERSION_PROBE_TIMEOUT_SECONDS = getenv("P99_UI_SING_BOX_VERSION_PROBE_TIMEOUT_SECONDS") || "1";
const SING_BOX_VERSION_PROBE_FAILURE_TTL_SECONDS = getenv("P99_UI_SING_BOX_VERSION_PROBE_FAILURE_TTL_SECONDS") || "30";
const ACTION_FINISHED_TTL_MINUTES = getenv("P99_UI_ACTION_FINISHED_TTL_MINUTES") || "60";
const ACTION_ACKED_TTL_SECONDS = getenv("P99_UI_ACTION_ACKED_TTL_SECONDS") || "15";
const ACTION_STALE_GRACE_SECONDS = getenv("P99_UI_ACTION_STALE_GRACE_SECONDS") || "15";
const SERVICE_ACTION_TIMEOUT_SECONDS = getenv("P99_UI_SERVICE_ACTION_TIMEOUT_SECONDS") || "120";
const SERVICE_ACTION_SETTLE_SECONDS = getenv("P99_UI_SERVICE_ACTION_SETTLE_SECONDS") || "2";
const RUNTIME_STABLE_MIN_AGE = getenv("P99_RUNTIME_STABLE_MIN_AGE") || "2";
const NFT_TABLE_NAME = getenv("NFT_TABLE_NAME") || "P99Table";
const RT_TABLE_NAME = getenv("RT_TABLE_NAME") || "p99";
...
function ui_state_json() {
    ...
    let p99_running = arg_number(ARGV[1]);
    let p99_enabled = arg_number(ARGV[2]);
    let p99_status = as_string(ARGV[3]);
    let p99_dns_configured = arg_number(ARGV[4]);
    let sing_box_running = arg_number(ARGV[5]);
    let sing_box_enabled = arg_number(ARGV[6]);
    let sing_box_status = as_string(ARGV[7]);

    if (p99_status == "")
        p99_status = service_status_text(p99_running, p99_enabled);
    ...
    write_json({
        service: {
            p99: {
                ...
                enabled: p99_enabled,
                status: p99_status,
                dns_configured: p99_dns_configured
            },
            sing_box: {
...
function current_ui_state_json() {
    ...
    let p99_is_running = p99_running() ? 1 : 0;
    let p99_is_enabled = service_enabled() ? 1 : 0;
    let sing_box_is_running = p99_is_running ? 1 : (sing_box_running() ? 1 : 0);
    let sing_box_is_enabled = sing_box_enabled() ? 1 : 0;
    let p99_status = service_status_text(p99_is_running, p99_is_enabled);
    ...
    if (active_action == "start")
        ...
        p99_status = "restarting";
    else if (active_action == "reload")
        p99_status = "reloading";

    write_json({
        service: {
            p99: {
                running: p99_is_running,
                enabled: p99_is_enabled,
                status: p99_status,
...
function service_action_reached_expected_state(action) {
    ...
    if (expected < 0)
        return false;
    return expected == 1 ? p99_running() : !p99_running();
}
...
function launch_worker(args) {
    ...
    let command = command_env({
        ...
        P99_SERVICE_NAME: SERVICE_NAME,
        P99_UI_STATE_DIR: STATE_DIR,
        P99_UI_SERVICE_ACTION_DIR: SERVICE_ACTION_DIR,
        P99_UI_SERVICE_ACTION_LOCK_DIR: SERVICE_ACTION_LOCK_DIR,
        P99_UI_LATENCY_ACTION_DIR: LATENCY_ACTION_DIR,
        P99_UI_COMPONENT_ACTION_DIR: COMPONENT_ACTION_DIR,
        P99_UI_SUBSCRIPTION_ACTION_DIR: SUBSCRIPTION_ACTION_DIR,
        P99_UI_SING_BOX_VERSION_CACHE_FILE: SING_BOX_VERSION_CACHE_FILE,
        P99_UI_SING_BOX_VERSION_CACHE_LOCK_DIR: SING_BOX_VERSION_CACHE_LOCK_DIR,
        P99_UI_SING_BOX_VARIANT_STATE_FILE: SING_BOX_VARIANT_STATE_FILE,
        P99_UI_SING_BOX_BIN_PATH: SING_BOX_BIN_PATH,
        P99_UI_SING_BOX_VERSION_PROBE_TIMEOUT_SECONDS: SING_BOX_VERSION_PROBE_TIMEOUT_SECONDS,
        P99_UI_SING_BOX_VERSION_PROBE_FAILURE_TTL_SECONDS: SING_BOX_VERSION_PROBE_FAILURE_TTL_SECONDS,
        P99_UI_ACTION_FINISHED_TTL_MINUTES: ACTION_FINISHED_TTL_MINUTES,
        P99_UI_ACTION_ACKED_TTL_SECONDS: ACTION_ACKED_TTL_SECONDS,
        P99_UI_ACTION_STALE_GRACE_SECONDS: ACTION_STALE_GRACE_SECONDS,
        P99_UI_SERVICE_ACTION_TIMEOUT_SECONDS: SERVICE_ACTION_TIMEOUT_SECONDS,
        P99_UI_SERVICE_ACTION_SETTLE_SECONDS: SERVICE_ACTION_SETTLE_SECONDS,
...
function run_pending_reload_after_service_action(action, success) {
    ...
    command_success_from_args([ "logger", "-t", SERVICE_NAME, "[info] Applying pending P99 reload" ]);
...
function finish_service_action_after_command(action, job_id_value, status, spawn_waiter) {
    ...
    if (!service_enabled() && !p99_running()) {
...
function service_action_worker(path, action, job_id_value, reason) {
    ...
    if (reason != "")
        push(args, reason);
    let command = "P99_UI_ACTION_TRACKED=1 " + command_from_args(args) + " >/dev/null 2>&1";
