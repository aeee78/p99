...
const STATE_FILE = getenv("P99_DNS_FAILOVER_STATE_FILE") || RUNTIME_STATE_DIR + "/dns-failover.json";
const PID_FILE = getenv("P99_DNS_FAILOVER_PID_FILE") || RUNTIME_STATE_DIR + "/dns-failover.pid";
...
function log_message(message, level) {
    command_success_from_args([ "logger", "-t", "p99", "[" + as_string(level || "info") + "] DNS failover: " + as_string(message) ]);
}
...
if (mode == "start-runtime")
    ...
    warn("Usage: singbox/dns_failover.uc <start-runtime|stop-runtime|worker|verify-state|commit-state|select-fixture|confirmation-fixture|verification-plan-fixture> ...\n");
    exit(1);
}
