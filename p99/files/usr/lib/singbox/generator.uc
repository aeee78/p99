...
function subscription_profile_group_outbound(outbound) {
    return subscription_urltest_group_outbound(outbound) && internal_flag(outbound.__p99_profile_group);
}
...
function subscription_hidden_outbound(outbound, refs, hide_urltest_group_outbounds, hide_detour_outbounds) {
    ...
    if (internal_flag(outbound.__p99_profile_member))
        return true;
...
function copy_subscription_outbound(outbound, new_tag) {
    ...
    for (let key, value in outbound) {
        if (key == "tag" || key == "remark" || key == "share_link" || substr(key, 0, 2) == "__")
            continue;
        copy[key] = value;
    }
...
function add_subscription_source_with_state(config, section, source_index, source_entry, taken, selector_tags, urltest_candidate_tags, state, show_metadata, include_urltest_groups, hide_urltest_group_outbounds, hide_detour_outbounds, node_prefix, dedup_cache) {
    ...
    for (let i = 0; i < length(outbounds); i++) {
        ...
        if (include_urltest_groups === false &&
            subscription_urltest_group_outbound(outbound) &&
            !subscription_profile_group_outbound(outbound))
            continue;
