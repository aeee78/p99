...
function subscription_group_outbound(outbound) {
    ...
    return (t == "selector" || t == "urltest") && internal_flag(outbound.__p99_allow_group);
}

function subscription_urltest_group_outbound(outbound) {
    if (type(outbound) != "object")
        return false;
    return as_string(outbound.type || "") == "urltest" && internal_flag(outbound.__p99_allow_group);
}
...
function subscription_hidden_outbound(outbound, refs, hide_urltest_group_outbounds, hide_detour_outbounds) {
    ...
    if (hidden_by_detour && hide_detour_outbounds !== false)
        return true;
    return internal_flag(outbound.__p99_hidden) && !hidden_by_urltest && !hidden_by_detour;
}
...
function copy_subscription_outbound(outbound, new_tag) {
    ...
    for (let key, value in outbound) {
        if (key != "tag" && key != "remark" && key != "share_link" &&
            key != "__p99_hidden" && key != "__p99_allow_group" &&
            key != "__p99_description")
            copy[key] = value;
    }
...
function add_urltest_outbound(config, section, urltest_id, urltest_candidate_tags, state) {
    ...
    if (length(urltest_outbounds) == 0 && length(urltest_candidate_tags) > 0) {
        warn("URLTest group '", display_name, "' for rule '", section_name, "' matched 0 outbounds after filtering; falling back to all candidate outbounds to prevent service outage\n");
        urltest_outbounds = urltest_all_candidate_outbounds(urltest_candidate_tags);
    }
