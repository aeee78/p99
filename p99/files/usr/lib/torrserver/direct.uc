...
const TABLE = "P99TorrServerDirect";
...
function rule_output_active(output, info) {
    ...
    return (index(output, "type route hook output priority mangle - 1") >= 0 ||
        ...
        index(output, "P99 TorrServer Direct") >= 0;
}
...
function apply_rule(info) {
    ...
    remove_rule();
    let ruleset_path = "/tmp/p99-torrserver-direct.nft";
    let ruleset = "add table inet " + TABLE + "\n" +
        ...
        "\" meta mark set " + OUTBOUND_MARK + " counter comment \"P99 TorrServer Direct\"\n";
