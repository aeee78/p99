...
UPDATES_UC="$P99_LIB/components/updates.uc"
WORK_DIR="$(mktemp -d)"
...
updates_ucode() {
  TEST_COMPONENT_UPDATE_CHECK_ENABLED="${TEST_COMPONENT_UPDATE_CHECK_ENABLED:-1}" \
    ...
    P99_LIB="$fake_lib" \
    P99_COMPONENT_UPDATE_CHECK_CACHE_DIR="$cache_dir" \
    P99_COMPONENT_UPDATE_CHECK_STATE_FILE="$timestamp_file" \
    UPDATES_JOB_DIR="$job_dir" \
    ucode -L "$fake_lib" -L "$P99_LIB" "$UPDATES_UC" "$@"
}
...
[ -s "$cache_dir/p99.json" ] ||
  fail "automatic checks must cache the P99 result"
[ -s "$timestamp_file" ] ||
