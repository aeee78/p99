...
CLI_UC="$P99_BIN"
UPDATER="$ROOT_DIR/p99/files/usr/lib/components/updater.uc"
UPDATES_UC="$ROOT_DIR/p99/files/usr/lib/components/updates.uc"
...
[ ! -e "$ROOT_DIR/p99/files/usr/lib/updater.sh" ] ||
...
grep -Fq 'p99_status_running_with_timeout()' "$ACTION_UC" ||
  fail "components/action.uc must use bounded P99 status checks for component actions"
...
grep -Fq 'fail_package_sing_box_install(action, tiny, "was installed, but P99 did not start cleanly"' "$ACTION_UC" ||
...
status_timeout_line="$(grep -n '^function p99_status_running_with_timeout()' "$ACTION_UC" | cut -d: -f1)"
capture_state_line="$(grep -n '^function capture_p99_running_state()' "$ACTION_UC" | cut -d: -f1)"
[ -n "$status_timeout_line" ] && [ -n "$capture_state_line" ] && [ "$status_timeout_line" -lt "$capture_state_line" ] ||
  fail "components/action.uc must declare bounded status helper before capture_p99_running_state for OpenWrt ucode"
...
prev1 == "    stop_p99_before_sing_box_change();" &&
...
grep -Fq 'backup_binary = backup_on_tmpfs ? tmp_dir + "/sing-box.p99-backup"' "$ACTION_UC" ||
...
P99_LIB="$package_runtime_lib" \
P99_RUNTIME_STATE_DIR="$WORK_DIR/package-runtime" \
P99_BIN="$WORK_DIR/missing-p99" \
P99_SERVICE_INIT="$WORK_DIR/missing-init" \
...
PATH="$package_runtime_bin:$PATH" \
P99_LIB="$package_runtime_lib" \
P99_RUNTIME_STATE_DIR="$WORK_DIR/apk-package-runtime" \
P99_BIN="$WORK_DIR/missing-p99" \
P99_SERVICE_INIT="$WORK_DIR/missing-init" \
...
release_json="$(cat <<'JSON'
{
  ...
  "assets": [
    {"name": "p99_1.2.3.ipk", "browser_download_url": "https://example.com/backend.ipk"},
    {"name": "luci-app-p99_1.2.3.ipk", "browser_download_url": "https://example.com/app.ipk"},
    {"name": "luci-i18n-p99-ru_1.2.3.ipk", "browser_download_url": "https://example.com/i18n.ipk"}
  ]
}
JSON
)"

assert_eq "$(printf 'https://example.com/release\tp99_1.2.3.ipk\thttps://example.com/backend.ipk\tluci-app-p99_1.2.3.ipk\thttps://example.com/app.ipk\tluci-i18n-p99-ru_1.2.3.ipk\thttps://example.com/i18n.ipk')" \
  "$(printf '%s' "$release_json" | ucode "$UPDATER" p99-release-plan 1.2.3 ipk 1)" \
  "p99 release plan with i18n"
assert_eq "$(printf 'https://example.com/release\tp99_1.2.3.ipk\thttps://example.com/backend.ipk\tluci-app-p99_1.2.3.ipk\thttps://example.com/app.ipk\t\t')" \
  "$(printf '%s' "$release_json" | ucode "$UPDATER" p99-release-plan 1.2.3 ipk 0)" \
  "p99 release plan without i18n"
if printf '%s' "$release_json" | ucode "$UPDATER" p99-release-plan 9.9.9 ipk 0 >/dev/null 2>&1; then
  fail "p99 release plan should reject tag mismatch"
fi
...
if printf '%s' "$missing_i18n_json" | ucode "$UPDATER" p99-release-plan 1.2.3 ipk 1 >/dev/null 2>&1; then
  fail "p99 release plan should require i18n asset when requested"
fi

ucode "$UPDATER" p99-release-version-valid 1.2.3 ||
  fail "three-part P99 release version should be valid"
for invalid_version in v1.2.3 1.2 1.2.3.4 1.2.3-1 1.2.3-r1; do
  if ucode "$UPDATER" p99-release-version-valid "$invalid_version" >/dev/null 2>&1; then
    fail "invalid P99 release version was accepted: $invalid_version"
  fi
done
assert_eq "-1" "$(ucode "$UPDATER" p99-release-version-compare 1.2.3 1.2.4)" \
  "three-part P99 release version comparison"
...
cp "$P99_LIB/core/uci.uc" "$fake_lib/core/uci.uc"
cp "$P99_LIB/core/common.uc" "$fake_lib/core/common.uc"
cp "$P99_LIB/core/constants.uc" "$fake_lib/core/constants.uc"
...
start_json="$(P99_LIB="$fake_lib" UPDATES_JOB_DIR="$component_actions_dir" \
  ucode -L "$P99_LIB" "$UPDATES_UC" component-action-async sing_box check_update)"
...
for _ in 1 2 3 4 5; do
  status_json="$(P99_LIB="$fake_lib" UPDATES_JOB_DIR="$component_actions_dir" \
    ucode -L "$P99_LIB" "$UPDATES_UC" component-action-status "$job_id")"
...
start_json="$(P99_LIB="$fake_lib" UPDATES_JOB_DIR="$component_actions_dir" \
  ucode -L "$P99_LIB" "$UPDATES_UC" component-action-async sing-box check_update)"
job_id="$(JSON_VALUE="$start_json" node - <<'NODE'
const value = JSON.parse(process.env.JSON_VALUE);
if (!value.success || !value.job_id) {
  process.exit(1);
