...
JS
  P99_LIB="$P99_LIB" ucode -L "$P99_LIB" "$VALIDATOR" validate-runtime-fixture "$normalized" "$context"
}
...
JSON
if output="$(P99_LIB="$P99_LIB" ucode -L "$P99_LIB" "$VALIDATOR" validate-runtime-fixture "$WORK_DIR/bad-byedpi.json" "$provider_context" 2>/dev/null)"; then
...
mkdir -p "$runtime_lib"
ln -s "$P99_LIB/core" "$runtime_lib/core"
ln -s "$P99_LIB/config" "$runtime_lib/config"
ln -s "$P99_LIB/subscription" "$runtime_lib/subscription"
ln -s "$P99_LIB/providers" "$runtime_lib/providers"
