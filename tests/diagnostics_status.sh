...
DIAGNOSTICS="$ROOT_DIR/p99/files/usr/lib/diagnostics/status.uc"
DIAGNOSTICS_RUNTIME="$ROOT_DIR/p99/files/usr/lib/diagnostics/runtime.uc"
...
[ ! -e "$P99_LIB/status_diagnostics.sh" ] ||
...
[ "$(P99_VERSION=runtime-test ucode -L "$P99_LIB" "$DIAGNOSTICS_RUNTIME" show-version)" = "runtime-test" ] ||
...
grep -Fq '"p99-stably-running", RT_TABLE_NAME, NFT_TABLE_NAME, NFT_FAKEIP_MARK, RUNTIME_STABLE_MIN_AGE' "$DIAGNOSTICS_RUNTIME" ||
  fail "diagnostics P99 status must use stable runtime state to avoid crash-loop flicker"
...
masked_config="$WORK_DIR/p99-masked"
...
EOF
masked_output="$(status_ucode p99-config-masked "$masked_config")"
case "$masked_output" in
  *device-secret*|*vless://secret*|*token=secret*|*user:password*) fail "masked P99 config leaked a secret" ;;
esac
case "$masked_output" in
  *"option hwid 'MASKED'"*) ;;
  *) fail "masked P99 config must preserve the HWID option shape" ;;
esac
case "$masked_output" in
  *"option url 'MASKED'"*) ;;
  *) fail "masked P99 config must mask subscription section URLs" ;;
esac
...
{
  printf 'Tue Jun 30 11:00:00 2026 user.notice p99: [info] Starting P99\n'
  ...
  printf 'Tue Jun 30 11:01:00 2026 user.notice p99: [info] large logread marker survived stdin transport\n'
} >"$WORK_DIR/large-logread.txt"
large_logs="$(P99_LIB="$P99_LIB" ucode -L "$P99_LIB" "$DIAGNOSTICS_RUNTIME" p99-logs-fixture <"$WORK_DIR/large-logread.txt")" ||
...
p99.settings.latency_test_url=https://latency.example/generate_204
EOF
FAKE_CURL_LOG="$WORK_DIR/fake-curl.log" \
P99_UCI_STATE_FILE="$uci_state" \
P99_LIB="$P99_LIB" \
PATH="$fake_bin:$PATH" \
  ucode -L "$P99_LIB" "$DIAGNOSTICS_RUNTIME" clash-api get_proxy_latency proxy-out 5000 >/dev/null ||
...
PATH="$fake_bin:$PATH" \
  ucode -L "$P99_LIB" "$DIAGNOSTICS_RUNTIME" clash-api get_proxy_latencies '["urltest","proxy-a","provider-urltest","proxy-b"]' 5000 "$latency_state" >/dev/null ||
...
P99_UCI_STATE_FILE="$uci_state" \
P99_LIB="$P99_LIB" \
P99_AUTOMATIC_LATENCY_TEST_LOCK_DIR="$WORK_DIR/automatic-latency-test.lock" \
PATH="$fake_bin:$PATH" \
  ucode -L "$P99_LIB" "$DIAGNOSTICS_RUNTIME" automatic-latency-test >/dev/null ||
...
printf '%s\n' "$sing_box_netstat" |
  P99_LIB="$P99_LIB" ucode -L "$P99_LIB" "$DIAGNOSTICS_RUNTIME" sing-box-standard-ports-listening-fixture >/dev/null ||
  fail "sing-box standard listeners should satisfy diagnostics"
if printf '%s\n' "$sing_box_netstat" | sed '/0.0.0.0:1602/d' |
  P99_LIB="$P99_LIB" ucode -L "$P99_LIB" "$DIAGNOSTICS_RUNTIME" sing-box-standard-ports-listening-fixture >/dev/null 2>&1; then
