...
GENERATOR="$P99_LIB/singbox/generator.uc"
FAILOVER="$P99_LIB/singbox/dns_failover.uc"
WORK_DIR="$(mktemp -d)"
...
generate() {
  ...
  P99_LIB="$P99_LIB" \
    P99_DNS_FAILOVER_STATE_FILE="$state" \
    ucode -L "$P99_LIB" "$GENERATOR" generate-config-fixture "$fixture" "$output" 192.168.1.1 0
}
...
selected="$(ucode -L "$P99_LIB" "$FAILOVER" select-fixture "$WORK_DIR/select-state.json" "$WORK_DIR/alive-failover.json" main 0)"
...
selected="$(ucode -L "$P99_LIB" "$FAILOVER" select-fixture "$WORK_DIR/select-state.json" "$WORK_DIR/alive-recovery.json" bootstrap 1)"
...
verification="$(ucode -L "$P99_LIB" "$FAILOVER" verification-plan-fixture "$WORK_DIR/verify-previous.json" "$WORK_DIR/verify-bootstrap.json")"
