...
function extractConfigDefaults(repo, fields) {
  const currentFile = path.join(repo, "p99", "files", "etc", "config", "p99");
...
function extractBackend(repo, fields) {
  const backendRoot = fs.existsSync(path.join(repo, "p99")) ? "p99" : legacyStem;
  const roots = [
  ...
  const files = roots
    ...
    .filter((file) => /\.(sh|uc)$/.test(file) || ["p99", legacyStem].includes(path.basename(file)));
  ...
  const migrationRe = new RegExp(`\\b(?:p99|${legacyStem})_uci_(?:set_option|set_option_if_missing|delete_option|add_list_unique)\\s+["'$A-Za-z0-9_{}.-]+\\s+["']([A-Za-z0-9_]+)["']`, "g");
