...
COMPONENT_ACTION_UC="$P99_LIB/components/action.uc"
DIAGNOSTICS_RUNTIME_UC="$P99_LIB/diagnostics/runtime.uc"
WORK_DIR="$(mktemp -d)"
...
if grep -R -n -F 'helpers.sh' "$P99_BIN" "$P99_LIB" --include='*.sh' >/dev/null 2>&1; then
...
for shell_owner_pattern in \
  ...
  'uci_set'
do
  if grep -Fq "$shell_owner_pattern" "$P99_BIN"; then
    fail "p99 shell entrypoint must not own $shell_owner_pattern logic"
  fi
done
if grep -E -n '(^|[;&|[:space:]])nft[[:space:]]+(list|add|delete|flush)' "$P99_BIN" >/dev/null 2>&1; then
  fail "p99 shell entrypoint must not own nft command logic"
fi
if grep -E -n '(^|[;&|[:space:]])ip[[:space:]]+(rule|route)' "$P99_BIN" >/dev/null 2>&1; then
  fail "p99 shell entrypoint must not own ip rule/route logic"
fi
[ ! -e "$P99_LIB/updater.sh" ] ||
