...
sed \
  -e '/^main "\$@"$/d' \
  -e 's#\[ -f /etc/openwrt_release \]#\[ -f "$P99_TEST_RELEASE_FILE" \]#' \
  "$ROOT_DIR/install.sh" > "$WORK_DIR/install-library.sh"
# shellcheck disable=SC1090
. "$WORK_DIR/install-library.sh"

P99_TEST_RELEASE_FILE="$WORK_DIR/openwrt_release"
touch "$P99_TEST_RELEASE_FILE"
...
P99_BACKEND_FILE="$WORK_DIR/backend.ipk"
P99_APP_FILE="$WORK_DIR/app.ipk"
P99_I18N_FILE=""
pkg_is_installed() { return 0; }
calculated_space="$(p99_install_required_space_kb)"
