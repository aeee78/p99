...
export P99_DEADLINE_HELPER_PATH="$deadline_helper"
export P99_INSTALLER_DEADLINE_HELPER="$deadline_helper"
export P99_INSTALLER_COMMAND_RESULT="$WORK_DIR/installer-command"
...
grep -Fq '/usr/bin/p99 package_postinst' "$INSTALLER" ||
  fail "installer must recover and validate the backend configuration after package installation"
grep -Fq 'P99 configuration recovery or validation failed' "$INSTALLER" ||
  fail "installer must fail clearly when backend configuration recovery fails"

grep -Fq '/usr/bin/p99 component_action sing_box "$action"' "$INSTALLER" ||
  fail "selected sing-box install must delegate to p99 component_action"
...
grep -Fq 'ucode -L /usr/lib/p99 /usr/lib/p99/config/migration.uc migrate-podkop' "$INSTALLER" ||
  fail "install.sh must delegate the legacy transition to the installed migration module"
grep -Fq 'ucode -L /usr/lib/p99 /usr/lib/p99/config/validator.uc validate-runtime' "$INSTALLER" ||
  fail "installer must validate the installed configuration after legacy migration"
grep -Fq 'ucode -L /usr/lib/p99 /usr/lib/p99/config/validator.uc check-requirements' "$INSTALLER" ||
  fail "installer must verify that migrated connection rules have a usable source"

if grep -n -E 'restore_p99_dnsmasq_failsafe|remember_service_state|stop_conflicting_services|deactivate_original_p99_if_present|remove_conflicting_dns_proxy|pkg_remove_if_installed|pkg_remove_matching_prefix|pkg_list_installed_names' "$INSTALLER" >/dev/null 2>&1; then
  fail "install.sh must not keep shell cleanup/remove service owners"
fi
if grep -n -E 'rm -f /var/luci-indexcache|rm -f /tmp/luci-indexcache|/etc/init\.d/rpcd[[:space:]]+reload|/etc/init\.d/p99[[:space:]]+(start|stop|disable|enable|restart)|/etc/init\.d/p99[[:space:]]+(stop|disable)' "$INSTALLER" >/dev/null 2>&1; then
...
awk '
  ...
  in_main && /resolve_p99_release/ { resolve = NR }
  in_main && /download_p99_packages/ { download = NR }
...
release_json='{"tag_name":"0.0.1","assets":[{"name":"p99_0.0.1.ipk","browser_download_url":"https://example.com/p99.ipk"}]}'
printf '%s' "$release_json" | ucode "$helper" release-asset-url backend ipk | grep -Fxq 'https://example.com/p99.ipk' ||
  fail "embedded helper must resolve the exact three-part P99 package name"
release_hash='0123456789abcdef0123456789abcdef0123456789abcdef0123456789abcdef'
printf '%s' "{\"tag_name\":\"0.0.1\",\"assets\":[{\"name\":\"p99_0.0.1.ipk\",\"browser_download_url\":\"https://example.com/p99.ipk\",\"sha256\":\"$release_hash\"}]}" |
...
if printf '%s' '{"tag_name":"0.0.1","assets":[{"name":"p99_0.0.1_all.ipk","browser_download_url":"https://example.com/old.ipk"}]}' |
  ucode "$helper" release-asset-url backend ipk | grep -q .; then
  fail "embedded helper must reject package names outside the P99 release format"
fi
...
case "$1" in
  list-installed)
    ...
    if [ -n "${P99_INSTALLER_FAKE_LEGACY_BACKEND:-}" ]; then
      printf '%s\n' \
        "$P99_INSTALLER_FAKE_LEGACY_BACKEND - 1.0" \
        "luci-app-$P99_INSTALLER_FAKE_LEGACY_BACKEND - 1.0" \
        "luci-i18n-$P99_INSTALLER_FAKE_LEGACY_BACKEND-ru - 1.0"
    fi
    ;;
  remove)
    shift
    [ "$1" = "--force-depends" ] && shift
    printf '%s\n' "$1" >> "$P99_INSTALLER_OPKG_LOG"
    [ "${P99_INSTALLER_FAIL_REMOVE:-}" != "$1" ] || exit 1
    ;;
esac
exit 0
SH
...
#!/usr/bin/env sh
printf '%s\n' "$1" >> "$P99_INSTALLER_INIT_LOG"
case "$1" in
  enabled|status|running)
    printf '%s\n' "$$" >> "$P99_INSTALLER_HANG_PID_LOG"
    trap '' TERM
    sh -c '
      trap "" TERM
      printf "%s\n" "$$" >> "$P99_INSTALLER_HANG_PID_LOG"
...
ORPHAN_PROBE_PID="$(
  P99_INSTALLER_INIT_LOG="$WORK_DIR/hanging-init.log" \
  P99_INSTALLER_HANG_PID_LOG="$HANG_PID_LOG" \
...
PATH="$WORK_DIR:$PATH" \
P99_INSTALLER_OPKG_LOG="$WORK_DIR/opkg.log" \
P99_INSTALLER_INIT="$WORK_DIR/hanging-init" \
P99_INSTALLER_BIN="$WORK_DIR/hanging-bin" \
P99_INSTALLER_LIB="$WORK_DIR/missing-p99-lib" \
P99_INSTALLER_UCI_DEFAULTS="$WORK_DIR/missing-uci-defaults" \
P99_INSTALLER_LUCI_VIEW="$WORK_DIR/missing-luci-view" \
P99_INSTALLER_MENU_JSON="$WORK_DIR/missing-menu.json" \
P99_INSTALLER_ACL_JSON="$WORK_DIR/missing-acl.json" \
P99_INSTALLER_RU_LMO="$WORK_DIR/missing-ru.lmo" \
P99_INSTALLER_EN_LMO="$WORK_DIR/missing-en.lmo" \
P99_INSTALLER_RU_LUA="$WORK_DIR/missing-ru.lua" \
P99_INSTALLER_EN_LUA="$WORK_DIR/missing-en.lua" \
P99_INSTALLER_LEGACY_BASE_INIT="$WORK_DIR/missing-original-init" \
P99_INSTALLER_LEGACY_BRAND="$LEGACY_BRAND" \
P99_INSTALLER_LEGACY_BACKEND="$LEGACY_BACKEND" \
P99_INSTALLER_RC_DIR="$WORK_DIR/rc.d" \
P99_INSTALLER_START_RETRY_FILE="$WORK_DIR/start.retry" \
P99_INSTALLER_START_RETRY_PID_FILE="$WORK_DIR/start-retry.pid" \
P99_INSTALLER_ORPHAN_PPID="$ORPHAN_PROBE_PPID" \
P99_INSTALLER_SERVICE_PROBE_TIMEOUT=1 \
P99_INSTALLER_SERVICE_ACTION_TIMEOUT=3 \
P99_INSTALLER_INIT_LOG="$WORK_DIR/hanging-init.log" \
P99_INSTALLER_HANG_PID_LOG="$HANG_PID_LOG" \
P99_UCI_STATE_FILE="$WORK_DIR/empty-uci.state" \
...
grep -Fxq 'P99_WAS_ENABLED=1' "$WORK_DIR/hanging-state.env" ||
...
printf '%s\n' '1' |
  PATH="$WORK_DIR:$PATH" \
  P99_INSTALLER_OPKG_LOG="$WORK_DIR/opkg.log" \
  P99_INSTALLER_INIT="$WORK_DIR/missing-p99-init" \
  P99_INSTALLER_BIN="$WORK_DIR/missing-p99-bin" \
  P99_INSTALLER_LIB="$WORK_DIR/missing-p99-lib" \
  P99_INSTALLER_UCI_DEFAULTS="$WORK_DIR/missing-uci-defaults" \
  P99_INSTALLER_LUCI_VIEW="$WORK_DIR/missing-luci-view" \
  P99_INSTALLER_MENU_JSON="$WORK_DIR/missing-menu.json" \
...
#!/usr/bin/env sh
printf '%s\n' "$*" >> "$P99_INSTALLER_BIN_LOG"
case "$1" in
  get_status) printf '%s\n' '{"running":1}' ;;
  restore_dnsmasq) exit "${P99_INSTALLER_FAIL_BACKEND_RESTORE:-0}" ;;
esac
exit 0
SH
...
P99_INSTALLER_BIN="$WORK_DIR/missing-p99-bin" \
P99_INSTALLER_LIB="$WORK_DIR/missing-p99-lib" \
P99_INSTALLER_UCI_DEFAULTS="$WORK_DIR/uci-defaults" \
P99_INSTALLER_LUCI_VIEW="$WORK_DIR/luci-view" \
P99_INSTALLER_MENU_JSON="$WORK_DIR/menu.json" \
P99_INSTALLER_ACL_JSON="$WORK_DIR/acl.json" \
P99_INSTALLER_RU_LMO="$WORK_DIR/ru.lmo" \
P99_INSTALLER_EN_LMO="$WORK_DIR/en.lmo" \
P99_INSTALLER_RU_LUA="$WORK_DIR/ru.lua" \
P99_INSTALLER_EN_LUA="$WORK_DIR/en.lua" \
P99_INSTALLER_LEGACY_BASE_INIT="$WORK_DIR/missing-original-init" \
P99_INSTALLER_LEGACY_BRAND="$LEGACY_BRAND" \
P99_INSTALLER_LEGACY_BACKEND="$LEGACY_BACKEND" \
P99_INSTALLER_LEGACY_CONFIG_ALT="$LEGACY_CONFIG_ALT" \
P99_INSTALLER_LEGACY_INIT="$WORK_DIR/legacy-init" \
P99_INSTALLER_LEGACY_BIN="$WORK_DIR/legacy-bin" \
P99_INSTALLER_LEGACY_LIB="$WORK_DIR/legacy-lib" \
P99_INSTALLER_FAKE_LEGACY_BACKEND="$LEGACY_BACKEND" \
P99_INSTALLER_OPKG_LOG="$WORK_DIR/opkg.log" \
P99_INSTALLER_INIT_LOG="$WORK_DIR/init.log" \
P99_INSTALLER_BIN_LOG="$WORK_DIR/bin.log" \
P99_INSTALLER_FAIL_BACKEND_RESTORE=1 \
P99_UCI_STATE_FILE="$WORK_DIR/empty-uci.state" \
  ucode "$helper" installer-cleanup-legacy >"$state"

grep -Fxq 'P99_WAS_ENABLED=1' "$state" ||
  fail "installer cleanup must export previous enabled state"
grep -Fxq 'P99_WAS_RUNNING=1' "$state" ||
  fail "installer cleanup must export previous running state"
grep -Fxq 'P99_LEGACY_DETECTED=1' "$state" ||
...
if printf '%s\n' '1' |
  PATH="$WORK_DIR:$PATH" \
  P99_INSTALLER_OPKG_LOG="$WORK_DIR/opkg.log" \
  P99_INSTALLER_FAIL_REMOVE="$LEGACY_BACKEND" \
  P99_INSTALLER_FAKE_LEGACY_BACKEND="$LEGACY_BACKEND" \
  P99_INSTALLER_LEGACY_BRAND="$LEGACY_BRAND" \
  P99_INSTALLER_LEGACY_BACKEND="$LEGACY_BACKEND" \
  P99_UCI_STATE_FILE="$WORK_DIR/empty-uci.state" \
...
p99_persistent="$WORK_DIR/p99-persistent"
...
mkdir -p \
  ...
  "$p99_persistent/tailscale/server-existing" \
...
printf '%s\n' 'current-p99-identity' >"$p99_persistent/tailscale/server-existing/node.key"
...
P99_INSTALLER_LEGACY_BACKEND="$LEGACY_BACKEND" \
P99_INSTALLER_LEGACY_CONFIG_ALT="$LEGACY_CONFIG_ALT" \
P99_INSTALLER_LEGACY_CONFIG="$legacy_config" \
P99_INSTALLER_LEGACY_CONFIG_FILE_ALT="$legacy_config_alt" \
P99_INSTALLER_LEGACY_PERSISTENT_DIR="$legacy_persistent" \
P99_INSTALLER_PERSISTENT_DIR="$p99_persistent" \
P99_INSTALLER_LEGACY_RUNTIME_DIR="$legacy_runtime" \
P99_INSTALLER_LEGACY_TMP_DIR="$legacy_tmp" \
P99_INSTALLER_LEGACY_TMP_ALT_DIR="$legacy_tmp_alt" \
P99_INSTALLER_LEGACY_BASE_CONFIG="$legacy_base_config" \
P99_INSTALLER_LEGACY_BASE_PERSISTENT_DIR="$legacy_base_persistent" \
P99_INSTALLER_LEGACY_BASE_RUNTIME_DIR="$legacy_base_runtime" \
P99_INSTALLER_LEGACY_BASE_TMP_DIR="$legacy_base_tmp" \
P99_INSTALLER_LEGACY_BASE_INIT="$legacy_base_init" \
P99_INSTALLER_LEGACY_BASE_BIN="$legacy_base_bin" \
P99_INSTALLER_LEGACY_BASE_LIB="$legacy_base_lib" \
P99_INSTALLER_LEGACY_BASE_UCI_DEFAULTS="$legacy_base_uci_defaults" \
P99_INSTALLER_LEGACY_BASE_LUCI_VIEW="$legacy_base_luci_view" \
P99_INSTALLER_LEGACY_BASE_MENU_JSON="$legacy_base_menu" \
P99_INSTALLER_LEGACY_BASE_ACL_JSON="$legacy_base_acl" \
P99_INSTALLER_LEGACY_BASE_I18N="$legacy_base_i18n" \
P99_INSTALLER_LEGACY_TMP_PACKAGE_GLOB="$WORK_DIR/*${LEGACY_BRAND}*" \
P99_INSTALLER_LEGACY_SCAN_ROOTS="$legacy_scan_root" \
  ucode "$helper" installer-finalize-legacy
[ "$(cat "$p99_persistent/tailscale/server-new/node.key")" = 'legacy-node-identity' ] ||
  fail "installer finalization must migrate legacy Tailscale node identity"
[ "$(cat "$p99_persistent/tailscale/server-existing/node.key")" = 'current-p99-identity' ] ||
  fail "installer finalization must not overwrite existing P99 Tailscale state"
...
p99_persistent_file="$WORK_DIR/p99-persistent-file"
touch "$p99_persistent_file"
if P99_INSTALLER_LEGACY_BRAND="$LEGACY_BRAND" \
  P99_INSTALLER_LEGACY_BACKEND="$LEGACY_BACKEND" \
  P99_INSTALLER_LEGACY_CONFIG_ALT="$LEGACY_CONFIG_ALT" \
  P99_INSTALLER_LEGACY_CONFIG="$legacy_config" \
  P99_INSTALLER_LEGACY_CONFIG_FILE_ALT="$legacy_config_alt" \
  P99_INSTALLER_LEGACY_PERSISTENT_DIR="$legacy_persistent" \
  P99_INSTALLER_PERSISTENT_DIR="$p99_persistent_file" \
  P99_INSTALLER_LEGACY_RUNTIME_DIR="$legacy_runtime" \
  P99_INSTALLER_LEGACY_TMP_DIR="$legacy_tmp" \
  P99_INSTALLER_LEGACY_TMP_ALT_DIR="$legacy_tmp_alt" \
  P99_INSTALLER_LEGACY_BASE_CONFIG="$legacy_base_config" \
  ...
  P99_INSTALLER_LEGACY_BASE_ACL_JSON="$legacy_base_acl" \
  P99_INSTALLER_LEGACY_BASE_I18N="$legacy_base_i18n" \
  P99_INSTALLER_LEGACY_TMP_PACKAGE_GLOB="$WORK_DIR/missing-${LEGACY_BRAND}*" \
  P99_INSTALLER_LEGACY_SCAN_ROOTS="$legacy_scan_root" \
...
write_fake_service_init "$WORK_DIR/p99-init"
touch "$WORK_DIR/luci-indexcache.one" "$WORK_DIR/luci-indexcache.two"
: >"$WORK_DIR/init.log"
P99_INSTALLER_INIT="$WORK_DIR/p99-init" \
P99_INSTALLER_RPCD_INIT="$WORK_DIR/missing-rpcd" \
P99_INSTALLER_LUCI_CACHE_GLOBS="$WORK_DIR/luci-indexcache*" \
P99_INSTALLER_LATEST_VERSION_CACHE="$WORK_DIR/latest.cache" \
P99_INSTALLER_SYSTEM_INFO_CACHE="$WORK_DIR/system-info.json" \
P99_INSTALLER_SERVER_COUNTRY_CACHE="$WORK_DIR/server-country.json" \
P99_INSTALLER_SING_BOX_VERSION_CACHE="$WORK_DIR/sing-box-version" \
P99_INSTALLER_TMP_SYSTEM_INFO_CACHE="$WORK_DIR/tmp-system-info.json" \
P99_WAS_ENABLED=1 \
P99_WAS_RUNNING=1 \
P99_INSTALLER_INIT_LOG="$WORK_DIR/init.log" \
...
P99_INSTALLER_INIT="$WORK_DIR/p99-init" \
P99_INSTALLER_RPCD_INIT="$WORK_DIR/missing-rpcd" \
P99_INSTALLER_LUCI_CACHE_GLOBS="$WORK_DIR/missing-luci-indexcache*" \
P99_INSTALLER_LATEST_VERSION_CACHE="$WORK_DIR/missing-latest.cache" \
P99_INSTALLER_SYSTEM_INFO_CACHE="$WORK_DIR/missing-system-info.json" \
P99_INSTALLER_SERVER_COUNTRY_CACHE="$WORK_DIR/missing-server-country.json" \
P99_INSTALLER_SING_BOX_VERSION_CACHE="$WORK_DIR/missing-sing-box-version" \
P99_INSTALLER_TMP_SYSTEM_INFO_CACHE="$WORK_DIR/missing-tmp-system-info.json" \
P99_WAS_ENABLED=1 \
P99_WAS_RUNNING=1 \
P99_CONFIG_READY=0 \
P99_INSTALLER_INIT_LOG="$WORK_DIR/init.log" \
...
if grep -Eq '^(enable|start|restart)$' "$WORK_DIR/init.log"; then
  fail "installer must not enable or start P99 when configuration validation failed"
fi
