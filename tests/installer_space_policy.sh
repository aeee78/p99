...
dd if=/dev/zero of="$WORK_DIR/p99.ipk" bs=1024 count=1 status=none
...
P99_BACKEND_FILE="$WORK_DIR/p99.ipk"
P99_APP_FILE="$WORK_DIR/app.ipk"
P99_I18N_FILE="$WORK_DIR/i18n.ipk"
...
[ "$(p99_install_required_space_kb)" -eq "$expected_required" ] ||
  fail_test "P99 space plan must be derived from the downloaded package sizes"
[ "$expected_required" -lt 15360 ] ||
  fail_test "calculated P99 space plan must replace the old fixed 15 MB threshold"

pkg_is_installed() { return 1; }
missing_dependency_required="$(p99_install_required_space_kb)"
...
[ "$(cat "$WORK_DIR/world-switch")" = sing-box ] ||
  fail_test "a stale sing-box-tiny APK world entry must force conversion before the P99 transaction"
: >"$APK_WORLD_FILE"
PKG_IS_APK=0
pkg_is_installed() { return 0; }
...
P99_TEST_PACKAGE_LOG="$WORK_DIR/package-manager.log"
export P99_TEST_PACKAGE_LOG
original_path="$PATH"
...
grep -Fxq 'remove --force-depends sing-box' "$P99_TEST_PACKAGE_LOG" ||
  fail_test "OPKG sing-box replacement must permit its dependency to be restored by tiny"
PKG_IS_APK=1
pkg_remove_name sing-box
grep -Fxq 'del --force-broken-world sing-box' "$P99_TEST_PACKAGE_LOG" ||
