...
PACKAGE_POT="$ROOT_DIR/luci-app-p99/po/templates/p99.pot"
CALLS_JSON="$ROOT_DIR/fe-app-p99/locales/calls.json"
...
if grep -Fq '_("Dismiss")' "$SECTION_JS"; then
  fail "P99 modals must use Close instead of the shared LuCI Dismiss key"
fi
grep -Fq '_("Close")' "$SECTION_JS" ||
  fail "P99 section settings modal must expose a Close action"
