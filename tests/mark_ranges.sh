...
VALIDATOR="$P99_LIB/config/validator.uc"
WORK_DIR="$(mktemp -d)"
...
P99_LIB="$P99_LIB" ucode -L "$P99_LIB" "$VALIDATOR" validate-runtime-fixture "$WORK_DIR/fixture.json" "$(context_json)"

if P99_LIB="$P99_LIB" ucode -L "$P99_LIB" "$VALIDATOR" validate-runtime-fixture "$WORK_DIR/fixture.json" "$(context_json "0x01010000")" >/dev/null 2>&1; then
  fail "legacy Zapret2 route mark base should overlap Tailscale fwmark mask"
fi
if P99_LIB="$P99_LIB" ucode -L "$P99_LIB" "$VALIDATOR" validate-runtime-fixture "$WORK_DIR/fixture.json" "$(context_json "" "0x00100000")" >/dev/null 2>&1; then
  fail "legacy FakeIP mark should overlap Tailscale fwmark mask"
fi
if P99_LIB="$P99_LIB" ucode -L "$P99_LIB" "$VALIDATOR" validate-runtime-fixture "$WORK_DIR/fixture.json" "$(context_json "" "" "0x00200000")" >/dev/null 2>&1; then
