...
P99_CONFIG="$ROOT_DIR/p99/files/etc/config/p99"
...
require_make_dep() {
  ...
  grep -Eq "DEPENDS:=.*(^|[[:space:]])\\+$package([[:space:]]|$)" "$P99_MAKEFILE" ||
    fail "p99/Makefile DEPENDS is missing +$package"
}
...
require_file "$P99_MAKEFILE"
require_file "$P99_CONFIG"
require_file "$BUILD_SCRIPT"
require_file "$BUILD_WORKFLOW"
require_file "$P99_LIB"
...
for conflict in https-dns-proxy nextdns luci-app-passwall luci-app-passwall2; do
  grep -E 'CONFLICTS:=' "$P99_MAKEFILE" | grep -Fq "$conflict" ||
    fail "p99/Makefile conflicts are missing $conflict"
...
if grep -Fq 'coreutils-sort' "$P99_MAKEFILE" "$BUILD_SCRIPT"; then
  fail "unused coreutils-sort runtime dependency must not be packaged"
fi

require_package_dependency "nftables-json"
if grep -Eq '(^|[[:space:],+])nftables([[:space:],]|$)' "$P99_MAKEFILE" "$BUILD_SCRIPT"; then
  fail "P99 must depend on the concrete nftables-json provider, not the nftables virtual package"
fi
grep -Fq "command_exists(\"nft\")" "$ROOT_DIR/p99/files/usr/lib/config/validator.uc" ||
  fail "runtime validation must reject a missing nft executable before applying rules"

grep -Fq "must use x.y.z format" "$P99_MAKEFILE" ||
  fail "p99/Makefile must enforce the three-part release version contract"
...
grep -Fq "option component_update_check_enabled '1'" "$P99_CONFIG" ||
  fail "new installations must enable component update checks by default"
grep -Fq "option config_version '1.0.5'" "$P99_CONFIG" ||
  fail "new installations must start at the current configuration schema version"
grep -Fq "list applied_migrations 'interface_sections'" "$P99_CONFIG" ||
  fail "new installations must mark the interface section migration as applied"
grep -Fq "list applied_migrations 'enable_component_checks'" "$P99_CONFIG" ||
  fail "new installations must mark the component check migration as applied"
grep -Fq "list applied_migrations 'http_connection_urls'" "$P99_CONFIG" ||
  fail "new installations must mark the HTTP connection URL migration as applied"
grep -Fq "list applied_migrations 'flintnet_urltest_default'" "$P99_CONFIG" ||
  fail "new installations must mark the Flintnet URLTest migration as applied"
grep -Fq '/usr/lib/p99/config/migration.uc migrate' "$P99_MAKEFILE" ||
  fail "OpenWrt package postinst must run configuration migrations"
[ "$(grep -Fc '/usr/lib/p99/config/migration.uc migrate' "$BUILD_SCRIPT")" -ge 3 ] ||
  fail "manual IPK/APK package scripts must run configuration migrations after install and upgrade"

if grep -Rqs 'require("uci")' "$P99_LIB"; then
  require_package_dependency "ucode-mod-uci"
fi

if grep -Rqs 'require("fs")' "$P99_LIB"; then
  require_package_dependency "ucode-mod-fs"
fi

if grep -Rqs 'p99_dnsmasq_failsafe_restore_raw' \
  "$ROOT_DIR/p99/files/usr/bin" \
  "$ROOT_DIR/p99/files/usr/lib" \
  "$ROOT_DIR/p99/files/etc/init.d"; then
