...
RULESET_CACHE_UC="$P99_LIB/singbox/ruleset_cache.uc"
WORK_DIR="$(mktemp -d)"
...
P99_RULESET_CACHE_DIR="$WORK_DIR/cache" \
P99_RULESET_CACHE_MANIFEST="$WORK_DIR/cache/manifest.json" \
  ucode -L "$P99_LIB" "$RULESET_CACHE_UC" materialize-config "$WORK_DIR/config.json"
...
P99_RULESET_CACHE_DIR="$WORK_DIR/cache" \
P99_RULESET_CACHE_MANIFEST="$WORK_DIR/cache/manifest.json" \
  ucode -L "$P99_LIB" "$RULESET_CACHE_UC" materialize-config "$WORK_DIR/offline.json" 2>/dev/null
ucode -e '
  let fs = require("fs");
...
P99_RULESET_CACHE_DIR="$WORK_DIR/cache" \
P99_RULESET_CACHE_MANIFEST="$WORK_DIR/cache/manifest.json" \
  ucode -L "$P99_LIB" "$RULESET_CACHE_UC" materialize-config "$WORK_DIR/cache-only.json" cache-only 2>/dev/null
...
fallback="$({
  P99_MIRROR_BASE_URL='https://mirror.test'
  SRS_MAIN_URL='https://mirror.test/p99/lists/rulesets/community'
  SRS_FALLBACK_MAIN_URL='https://upstream.test/community'
  export P99_MIRROR_BASE_URL SRS_MAIN_URL SRS_FALLBACK_MAIN_URL
  ucode -L "$P99_LIB" "$RULESET_CACHE_UC" fallback-urls \
    'https://mirror.test/p99/lists/rulesets/community/youtube.srs'
})"
...
P99_RULESET_CACHE_DIR="$WORK_DIR/cache" \
P99_RULESET_CACHE_MANIFEST="$WORK_DIR/cache/manifest.json" \
  ucode -L "$P99_LIB" "$RULESET_CACHE_UC" refresh >/dev/null 2>&1 || true
