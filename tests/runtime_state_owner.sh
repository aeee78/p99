...
P99_INIT="$ROOT_DIR/p99/files/etc/init.d/p99"
CLI_UC="$P99_BIN"
RUNTIME_STATE="$ROOT_DIR/p99/files/usr/lib/runtime_state.sh"
UPDATES_RUNTIME="$ROOT_DIR/p99/files/usr/lib/updates_runtime.sh"
LIFECYCLE_UC="$ROOT_DIR/p99/files/usr/lib/service/lifecycle.uc"
UPDATES_UC="$ROOT_DIR/p99/files/usr/lib/components/updates.uc"
STATE_UC="$ROOT_DIR/p99/files/usr/lib/service/state.uc"
NFT_UC="$ROOT_DIR/p99/files/usr/lib/nft/apply.uc"
...
grep -Fq 'runtime_state.sh' "$P99_BIN" &&
  fail "p99 must not source runtime_state.sh"

service_predicates='process_age_seconds|sing_box_service_pid|pid_is_sing_box|sing_box_service_is_running|sing_box_service_is_stable|p99_runtime_network_is_configured|p99_is_running|p99_is_stably_running|wait_for_p99_stable_start'
if grep -R -n -E "$service_predicates" "$P99_BIN" "$ROOT_DIR/p99/files/usr/lib" --include='*.sh' >/dev/null 2>&1; then
...
if grep -R -n -F 'updates_runtime.sh' "$P99_BIN" "$ROOT_DIR/p99/files/usr/lib" --include='*.sh' >/dev/null 2>&1; then
...
grep -Fq 'mode == "p99-running"' "$STATE_UC" ||
  fail "service/state.uc must own P99 running predicate"
grep -Fq 'mode == "sing-box-service-stable"' "$STATE_UC" ||
  fail "service/state.uc must expose stable sing-box predicate"
grep -Fq 'mode == "wait-p99-stable-start"' "$STATE_UC" ||
...
grep -Fq '"wait-p99-stable-start"' "$LIFECYCLE_UC" ||
...
grep -Fq 'Rule-set cache changed; reloading P99 before automatic latency test' "$LIFECYCLE_UC" ||
  fail "service/lifecycle.uc must reload changed rule-sets before cold-start latency tests"
grep -Fq 'Reload verification failed after sing-box was reloaded; stopping P99 runtime' "$LIFECYCLE_UC" ||
  fail "service/lifecycle.uc must fail reload when sing-box does not stay stable"
grep -Fq 'Reload runtime restart verification failed after P99 was started; rolling back DNS changes' "$LIFECYCLE_UC" ||
...
if grep -Fq 'nft-create-full-runtime-from-uci' "$LIFECYCLE_UC"; then
  fail "p99 start/reload must rebuild nft runtime instead of appending to stale state"
fi
