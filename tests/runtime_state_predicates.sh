...
export P99_FAKE_INIT_CAPTURE="$WORK_DIR/pending-reload-init.args"
...
state_ucode() {
  ucode -L "$P99_LIB" "$STATE_UC" "$@"
}

nft_ucode() {
  ucode -L "$P99_LIB" "$NFT_UC" "$@"
}
...
if ! PATH="$WORK_DIR/stable-start-bin:$PATH" \
  ...
  SING_BOX_TEST_PID_FILE="$WORK_DIR/sing-box.pid" \
  SING_BOX_TEST_NETSTAT_FILE="$WORK_DIR/sing-box.netstat" \
  state_ucode p99-running p99 P99Table 0x00100000; then
  kill "$sing_box_pid" >/dev/null 2>&1 || true
  wait "$sing_box_pid" 2>/dev/null || true
  fail "stable-start fixture must expose configured P99 networking"
fi
...
if PATH="$WORK_DIR/stable-start-bin:$PATH" \
  ...
  state_ucode p99-running p99 P99Table 0x00100000 >/dev/null 2>&1; then
  kill "$sing_box_pid" >/dev/null 2>&1 || true
  wait "$sing_box_pid" 2>/dev/null || true
...
if ! PATH="$WORK_DIR/stable-start-bin:$PATH" \
  ...
  SING_BOX_TEST_PID_FILE="$WORK_DIR/sing-box.pid" \
  SING_BOX_TEST_NETSTAT_FILE="$WORK_DIR/sing-box.netstat" \
  state_ucode wait-p99-stable-start p99 P99Table 0x00100000 2 2; then
  kill "$sing_box_pid" >/dev/null 2>&1 || true
  wait "$sing_box_pid" 2>/dev/null || true
...
0
[dhcp.@dnsmasq[0].p99_server]
127.0.0.42#53
[dhcp.@dnsmasq[0].p99_noresolv]
0
[dhcp.@dnsmasq[0].p99_cachesize]
1500
[dhcp.p99.present]
1
EOF_DNSMASQ_SIG
