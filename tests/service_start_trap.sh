...
INITD_UC="$ROOT_DIR/p99/files/usr/lib/service/initd.uc"
...
SUBSCRIPTION_CACHE_UC="$ROOT_DIR/p99/files/usr/lib/subscription/cache.uc"
NFQUEUE_RUNTIME_UC="$ROOT_DIR/p99/files/usr/lib/providers/nfqueue/runtime.uc"
BYEDPI_RUNTIME_UC="$ROOT_DIR/p99/files/usr/lib/providers/byedpi/runtime.uc"
PRIORITY_UC="$ROOT_DIR/p99/files/usr/lib/singbox/priority.uc"
...
reject_pattern() {
  ...
  ! grep -Fq "$pattern" "$P99_BIN" || fail "$label"
}
...
[ -r "$P99_BIN" ] || fail "p99 binary source is missing"
[ -r "$P99_BIN" ] || fail "p99 entrypoint is missing"
...
reject_pattern "trap " \
  "p99 shell entrypoint must not own startup fail-safe traps"
reject_pattern "clear_startup_failsafe_trap" \
  "p99 shell entrypoint must not keep old trap helper"
...
require_file_pattern "$P99_INIT" 'P99_LAST_START_STATUS="$?"' \
  "init.d start_service must preserve backend start status for rc.common"
require_file_pattern "$P99_INIT" 'service_started()' \
