...
CLI_UC="$P99_BIN"
SING_BOX_RUNTIME_SH="$P99_LIB/sing_box_runtime.sh"
LIFECYCLE_UC="$P99_LIB/service/lifecycle.uc"
SINGBOX_RUNTIME_UC="$P99_LIB/singbox/runtime.uc"
SINGBOX_GENERATOR_UC="$P99_LIB/singbox/generator.uc"
WORK_DIR="$(mktemp -d)"
...
if grep -R -n -E 'sing_box_runtime_ucode|rulesets_ucode|sing_box_configure_service|sing_box_init_config|get_service_listen_address|get_device_ipv4_address|get_download_detour_tag' "$P99_BIN" "$P99_LIB" --include='*.sh' >/dev/null 2>&1; then
...
grep -Fq 'P99_RULE_CONDITION_CACHE_DIR' "$LIFECYCLE_UC" ||
...
stable_variant="$({
  PATH="$WORK_DIR/bin:$PATH" \
    ...
    ucode -L "$P99_LIB" "$SINGBOX_RUNTIME_UC" variant
...
if PATH="$WORK_DIR/check-bin:$PATH" \
  ucode -L "$P99_LIB" "$SINGBOX_RUNTIME_UC" check-config-fixture \
...
generator_reason="$(ucode -L "$P99_LIB" "$SINGBOX_RUNTIME_UC" generator-failure-reason-fixture \
...
ucode -L "$P99_LIB" "$SINGBOX_RUNTIME_UC" save-config-file-fixture \
...
EOF_SECTION_CACHE
P99_SECTION_CACHE_DIR="$WORK_DIR/runtime-cache" \
  ucode -L "$P99_LIB" "$SINGBOX_RUNTIME_UC" publish-section-cache-fixture \
...
generate_config() {
  ...
  ucode -L "$P99_LIB" "$P99_LIB/singbox/generator.uc" generate-config-fixture \
...
generate_config_with_subscription_cache() {
  ...
  TMP_SUBSCRIPTION_FOLDER="$WORK_DIR/subscriptions" \
    P99_PERSISTENT_SUBSCRIPTION_CACHE_DIR="$WORK_DIR/persistent-subscription-cache" \
    ucode -L "$P99_LIB" "$P99_LIB/singbox/generator.uc" generate-config-fixture \
...
if ucode -L "$P99_LIB" "$SINGBOX_GENERATOR_UC" generate-config-fixture \
...
p99.settings.dns_server=1.1.1.1
p99.settings.bootstrap_dns_server=1.1.1.1
p99.settings.config_path=/tmp/sing-box/config.json
p99.settings.cache_path=/tmp/sing-box/cache.db
p99.settings.log_level=warn
p99.uci_proxy=section
p99.uci_proxy.enabled=1
p99.uci_proxy.action=connection
p99.uci_proxy.outbound_jsons={"type":"direct"}
p99.uci_proxy.domain_suffix=example.org
EOF_UCI
P99_UCI_STATE_FILE="$WORK_DIR/generator-uci.state" \
  P99_SECTION_CACHE_DIR="$WORK_DIR/generated-from-uci.section-cache" \
  ucode -L "$P99_LIB" "$SINGBOX_GENERATOR_UC" generate-config "$WORK_DIR/generated-from-uci.json" "127.0.0.1" "0"
