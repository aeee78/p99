...
normalize_link() {
  ...
  printf '%s\n' "$link" > "$input"
  ucode -L "$P99_LIB" "$PARSER" normalize-uri-list "$input" "$output"
  printf '%s\n' "$output"
}
...
JSON
ucode -L "$P99_LIB" "$PARSER" validate-subscription "$WORK_DIR/valid-reality.json" ||
...
JSON
if ucode -L "$P99_LIB" "$PARSER" validate-subscription "$WORK_DIR/invalid-reality.json"; then
...
proxies:
  ...
  - name: clash-http
    ...
    port: 8080
YAML
ucode -L "$P99_LIB" "$PARSER" normalize-clash-yaml "$clash_input" "$clash_output"
...
JSON
ucode -L "$P99_LIB" "$PARSER" normalize-content "$xray_input" "$xray_output"
...
ucode -L "$P99_LIB" "$PARSER" try-decode-gzip-content "$gzip_decoded"
assert_contains "$gzip_decoded" 'gzip-vless' "gzip decode in place"
ucode -L "$P99_LIB" "$PARSER" normalize-content-validated "$gzip_input" "$gzip_output"
