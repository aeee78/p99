...
CACHE_UC="$ROOT_DIR/p99/files/usr/lib/subscription/cache.uc"
...
SUBSCRIPTION_RUNTIME_SH="$ROOT_DIR/p99/files/usr/lib/subscription_runtime.sh"
UPDATES_RUNTIME_SH="$ROOT_DIR/p99/files/usr/lib/updates_runtime.sh"
...
if grep -R -n "subscription_runtime.sh" "$ROOT_DIR/p99/files" >/dev/null 2>&1; then
...
if grep -R -n -E "$subscription_runtime_shell_symbols" "$ROOT_DIR/p99/files/usr/bin/p99" "$ROOT_DIR/p99/files/usr/lib" --include='*.sh' >/dev/null 2>&1; then
  fail "subscription_runtime shell symbols must not remain in runtime shell"
fi

if grep -R -n -E 'get_subscription_metadata_path|get_outbound_metadata_path' "$ROOT_DIR/p99/files/usr/bin/p99" "$ROOT_DIR/p99/files/usr/lib" --include='*.sh' >/dev/null 2>&1; then
...
assert_eq "proxy-subscription-2" \
  ...
  "source id mode"
assert_eq "/var/run/p99/subscription-metadata/proxy.json" \
  "$(cache_ucode subscription-metadata-path proxy)" \
  "subscription metadata path mode"
assert_eq "/var/run/p99/outbound-metadata/proxy.json" \
...
runtime_env() {
  TMP_SING_BOX_FOLDER="$WORK_DIR/runtime/tmp-sing-box" \
    ...
    P99_RUNTIME_STATE_DIR="$WORK_DIR/runtime/run" \
    P99_SUBSCRIPTION_UPDATE_STATE_DIR="$WORK_DIR/runtime/run/subscription-update" \
    P99_SUBSCRIPTION_LINKS_DIR="$WORK_DIR/runtime/run/subscription-links" \
    P99_SUBSCRIPTION_METADATA_DIR="$WORK_DIR/runtime/run/subscription-metadata" \
    P99_OUTBOUND_METADATA_DIR="$WORK_DIR/runtime/run/outbound-metadata" \
    P99_SECTION_CACHE_DIR="$WORK_DIR/runtime/run/section-cache" \
    P99_RUNTIME_CACHE_FORMAT_FILE="$WORK_DIR/runtime/run/cache-format" \
    P99_PERSISTENT_SUBSCRIPTION_CACHE_DIR="$WORK_DIR/runtime/persistent" \
    P99_PERSISTENT_SUBSCRIPTION_CACHE_FORMAT_FILE="$WORK_DIR/runtime/persistent/cache-format" \
    P99_PERSISTENT_SUBSCRIPTION_CACHE_FORMAT="7" \
    P99_SUBSCRIPTION_BOOTSTRAP_RETRY_PID_FILE="$WORK_DIR/runtime/run/bootstrap.pid" \
    "$@"
}
