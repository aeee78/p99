...
P99_LIB="$ROOT_DIR/p99/files/usr/lib"
GENERATOR="$ROOT_DIR/p99/files/usr/lib/singbox/generator.uc"
WORK_DIR="$(mktemp -d)"
...
JSON
ucode -L "$P99_LIB" "$PARSER" normalize-content "$singbox_input" "$singbox_output"
...
JSON
ucode -L "$P99_LIB" "$PARSER" normalize-content "$singbox_missing_tls_input" "$singbox_missing_tls_output"
...
for fingerprint in randomizedalpn randomizednoalpn; do
  ...
  ucode -L "$P99_LIB" "$PARSER" normalize-uri-list "$WORK_DIR/$fingerprint.in" "$fingerprint_output"
...
TMP_SUBSCRIPTION_FOLDER="$WORK_DIR/subscriptions" \
  ucode -L "$P99_LIB" "$GENERATOR" generate-config-fixture \
