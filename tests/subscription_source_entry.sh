...
PARSER="$ROOT_DIR/p99/files/usr/lib/subscription/parser.uc"
WORK_DIR="$(mktemp -d)"
...
assert_tsv() {
  ...
  parsed="$(ucode -L "$P99_LIB" "$PARSER" parse-source-entry-tsv "$entry")"
  tab="$(printf '\t')"
  url="${parsed%%"$tab"*}"
  if [ "$url" = "$parsed" ]; then
    user_agent=""
...
assert_rejects() {
  ...
  if output="$(ucode -L "$P99_LIB" "$PARSER" parse-source-entry-tsv "$entry" 2>/dev/null)"; then
...
assert_tsv ' http://my-proxy-provider.org/sub ' 'http://my-proxy-provider.org/sub' ''
...
assert_rejects 'file:///tmp/sub.txt' 'Subscription URL must use HTTP or HTTPS'
...
if (invalid.valid || invalid.error != "Subscription URL must use HTTP or HTTPS")
    exit(1);
...
let any_provider = parser.parse_subscription_source_entry("https://internet.matryoshka.my/a");
if (!any_provider.valid || any_provider.url != "https://internet.matryoshka.my/a")
    exit(1);
UCODE

ucode -L "$P99_LIB" "$WORK_DIR/require-subscription-parser.uc"
