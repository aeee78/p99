...
SH
chmod +x "$WORK_DIR/bin/p99"

ASYNC_ENV=(
  "P99_LIB=$ROOT_DIR/p99/files/usr/lib"
  "P99_BIN=$WORK_DIR/bin/p99"
  ...
  "P99_RUNTIME_STATE_DIR=$WORK_DIR/run"
  "P99_SUBSCRIPTION_UPDATE_STATE_DIR=$WORK_DIR/run/subscription-update"
  "P99_SUBSCRIPTION_UPDATE_JOB_DIR=$WORK_DIR/run/subscription-update-jobs"
  "P99_SUBSCRIPTION_LINKS_DIR=$WORK_DIR/run/subscription-links"
  "P99_SUBSCRIPTION_METADATA_DIR=$WORK_DIR/run/subscription-metadata"
  "P99_OUTBOUND_METADATA_DIR=$WORK_DIR/run/outbound-metadata"
  "P99_SECTION_CACHE_DIR=$WORK_DIR/run/section-cache"
  "P99_RUNTIME_CACHE_FORMAT_FILE=$WORK_DIR/run/cache-format"
  "P99_PERSISTENT_SUBSCRIPTION_CACHE_DIR=$WORK_DIR/persistent/subscription-cache"
  "P99_PERSISTENT_SUBSCRIPTION_CACHE_FORMAT_FILE=$WORK_DIR/persistent/subscription-cache/cache-format"
)

async_response="$(env "${ASYNC_ENV[@]}" ucode -L "$ROOT_DIR/p99/files/usr/lib" "$UPDATES_UC" subscription-update-async proxy 1)"
...
for _ in $(seq 1 30); do
  final_status="$(env "${ASYNC_ENV[@]}" ucode -L "$ROOT_DIR/p99/files/usr/lib" "$UPDATES_UC" subscription-update-status "$job_id")"
