...
REAL_LIB="$ROOT_DIR/p99/files/usr/lib"
WORK_DIR="$(mktemp -d)"
...
run_update() {
  ...
  env \
    ...
    P99_SUBSCRIPTION_UPDATE_LOCK_DIR="$WORK_DIR/run/subscription-update.lock" \
    P99_RELOAD_LOCK_DIR="$WORK_DIR/run/reload.lock" \
    ...
    P99_PENDING_RELOAD_FILE="$WORK_DIR/run/reload.pending" \
    P99_RELOAD_STATE_FILE="$WORK_DIR/run/reload-state" \
    P99_RULE_CONDITION_CACHE_DIR="$WORK_DIR/run/rule-condition-cache" \
    FAKE_CALL_LOG="$log" \
