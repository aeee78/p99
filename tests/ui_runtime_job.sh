...
UI_UC="$ROOT_DIR/p99/files/usr/lib/service/ui.uc"
P99_FILES="$ROOT_DIR/p99/files"
P99_BIN="$P99_FILES/usr/bin/p99"
P99_INIT="$P99_FILES/etc/init.d/p99"
UI_RUNTIME_SH="$P99_FILES/usr/lib/ui_runtime.sh"
WORK_DIR="$(mktemp -d)"
...
ui_ucode() {
  ucode -L "$P99_FILES/usr/lib" "$UI_UC" "$@"
}
...
if grep -R -n "ui_runtime.sh" "$P99_FILES" >/dev/null 2>&1; then
  fail "runtime files must not reference ui_runtime.sh"
fi

ui_runtime_shell_symbols='ui_runtime_|p99_fast_get_ui_state|P99_UI_RUNTIME|load_ui_runtime'
if grep -R -n -E "$ui_runtime_shell_symbols" "$P99_BIN" "$P99_INIT" "$P99_FILES/usr/lib" --include='*.sh' >/dev/null 2>&1; then
  fail "ui_runtime shell symbols must not remain in runtime shell"
fi

if grep -R -n '^get_ui_capabilities()' "$P99_FILES/usr/lib" --include='*.sh' >/dev/null 2>&1; then
...
grep -Fq '"p99-stably-running"' "$UI_UC" ||
  fail "UI P99 status must use stable runtime state to avoid crash-loop flicker"
...
P99_UI_ACTION_FINISHED_TTL_MINUTES=60 ui_ucode cleanup-action-dir-fixture "$cleanup_dir"
[ ! -e "$old_state" ] ||
  fail "old finished action state should be cleaned without find"

export P99_UI_STATE_DIR="$WORK_DIR/ui-state"
export P99_UI_SERVICE_ACTION_DIR="$P99_UI_STATE_DIR/service-actions"
export P99_UI_SERVICE_ACTION_LOCK_DIR="$P99_UI_STATE_DIR/service-actions.lock"
export P99_UI_LATENCY_ACTION_DIR="$P99_UI_STATE_DIR/latency-actions"
export P99_UI_COMPONENT_ACTION_DIR="$P99_UI_STATE_DIR/component-actions"
export P99_UI_SUBSCRIPTION_ACTION_DIR="$P99_UI_STATE_DIR/subscription-actions"
...
latency_async_state="$P99_UI_LATENCY_ACTION_DIR/$latency_job_id.json"
...
service_state="$P99_UI_SERVICE_ACTION_DIR/$job_id.json"
...
P99_UI_SERVICE_ACTION_TIMEOUT_SECONDS=1 \
P99_UI_SERVICE_ACTION_SETTLE_SECONDS=0 \
P99_SERVICE_INIT=/bin/true \
P99_BIN=/bin/true \
P99_LIB="$P99_FILES/usr/lib" \
...
component_job="$P99_UI_COMPONENT_ACTION_DIR/component-1.json"
mkdir -p "$P99_UI_COMPONENT_ACTION_DIR"
