...
cat >"$PROBE_BIN" <<'SH'
#!/bin/sh
printf '%s\n' "$$" >>"$P99_TEST_SING_BOX_PROBE_PIDS"
printf 'probe\n' >>"$P99_TEST_SING_BOX_PROBE_COUNT"
if [ "${P99_TEST_SING_BOX_PROBE_MODE:-fast}" = "slow" ]; then
  exec sleep 30
fi
...
ui_capabilities() {
  PATH="$WORK_DIR:$PATH" \
  P99_CONFIG_NAME=p99-ui-probe-test \
  P99_UI_STATE_DIR="$WORK_DIR/state" \
  P99_UI_COMPONENT_ACTION_DIR="$WORK_DIR/components" \
  P99_UI_SING_BOX_VERSION_CACHE_FILE="$CACHE_FILE" \
  P99_UI_SING_BOX_VARIANT_STATE_FILE="$WORK_DIR/missing-variant" \
  P99_UI_SING_BOX_BIN_PATH="$PROBE_BIN" \
  P99_UI_SING_BOX_VERSION_PROBE_TIMEOUT_SECONDS=1 \
  P99_UI_SING_BOX_VERSION_PROBE_FAILURE_TTL_SECONDS=30 \
  ...
  P99_TEST_SING_BOX_PROBE_COUNT="$PROBE_COUNT" \
  P99_TEST_SING_BOX_PROBE_PIDS="$PROBE_PIDS" \
  ucode -L "$P99_LIB" "$UI_UC" get-ui-capabilities
}

fast_first="$(P99_TEST_SING_BOX_PROBE_MODE=fast ui_capabilities)"
fast_second="$(P99_TEST_SING_BOX_PROBE_MODE=fast ui_capabilities)"
...
for index in 1 2 3 4 5; do
  P99_TEST_SING_BOX_PROBE_MODE=slow ui_capabilities >"$WORK_DIR/slow-$index.json" &
  workers="$workers $!"
done
...
P99_TEST_SING_BOX_PROBE_MODE=slow ui_capabilities >/dev/null
[ "$(wc -l <"$PROBE_COUNT")" -eq 1 ] ||
