...
ZAPRET_RUNTIME="$P99_LIB/providers/zapret/runtime.uc"
ZAPRET2_RUNTIME="$P99_LIB/providers/zapret2/runtime.uc"
ZAPRET_COMMON="$P99_LIB/providers/zapret/common.uc"
ZAPRET2_COMMON="$P99_LIB/providers/zapret2/common.uc"
NFQUEUE_RUNTIME="$P99_LIB/providers/nfqueue/runtime.uc"
NFQUEUE_CHECK="$P99_LIB/providers/nfqueue/check.uc"
NFQUEUE_VALIDATOR="$P99_LIB/providers/nfqueue/validator.uc"
ZAPRET2_CHECK="$P99_LIB/providers/zapret2/check.uc"
ZAPRET2_VALIDATOR="$P99_LIB/providers/zapret2/validator.uc"
...
if grep -R -n -E 'providers/zapret/runtime\.uc" (start-runtime|stop-runtime|status|check|installed|package-installed|package-version|enabled-rule-count) zapret2' \
  "$P99_BIN" "$P99_LIB" --include='*.sh' >/dev/null 2>&1; then
  fail "zapret2 must not be multiplexed through providers/zapret/runtime.uc"
fi
if grep -R -n -E 'providers\.zapret\.|providers/zapret/' "$P99_LIB/providers/zapret2" >/dev/null 2>&1; then
...
status_json="$(P99_CONFIG_NAME=p99-definitely-missing ucode -L "$P99_LIB" "$ZAPRET_RUNTIME" status)"
...
check_json="$(ucode -L "$P99_LIB" "$ZAPRET2_RUNTIME" check)"
...
ucode -L "$P99_LIB" -- "$ZAPRET2_VALIDATOR" validate-json nfqws2 '--name p99 --intercept=1' >/dev/null ||
...
printf 'table inet x { chain y { queue num 4301 bypass } }\n' |
  ucode -L "$P99_LIB" "$ZAPRET2_CHECK" nft-queue-overlap P99Table 4300 4555 >/dev/null ||
